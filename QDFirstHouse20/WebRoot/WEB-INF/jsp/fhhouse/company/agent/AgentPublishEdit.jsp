	...
	<script type="text/javascript">
		...
		function reset(){
	 	...
	 	function doSave(){
	 		...
	 		if(a=="1" || a=="2"){
	 			window.location="<%=path%>/inner/agentmanage/publishSubmit.action?status="+a+"&agentID="+c;
	 		}else{
	 			alert("请输入信息发布意见后进行提交！");
	 			return;
	 		}
	 	}
...
<body id="body-layout" >
	...
	<table class="input_table" cellpadding="5" cellspacing="1px" style="width:100%;background-color: #7DD5FF">
		...
		<tr>
			<td class="input_left input_required" bgcolor="#5BADFF"><font style="color: white">信息发布：</font></td>
        	<td class="input_right" colspan="3">
				<label><input name="status" type="radio" value="1" />同意 </label> 
				<label><input name="status" type="radio" value="2" />不同意 </label> 
			</td> 
		</tr>
	</table>
