...
<html xmlns="http://www.w3.org/1999/xhtml">
	...
	<script type="text/javascript" language="javascript">
		//判断输入的合同代码是否可用
	 	function doModifyPwd(){
	 	  ...
	 	  var contractID = $("#contractID").textbox('getValue');
	 	  if(contractID=='') {
                alert("请输入合同代码!");
                $("input").focus();
                return;
	 		}else if(!reg.test(contractID)){
           		alert("请输入数字代码！");
	 		}else{
	 			...
	 			$.ajax({
					...
					url : "<%=path%>/inner/pwdmodify/findBuyer.action",
					data : "contractID="+contractID,
					dataType : "json",
					success : function(msg){
						if(msg[0].result=="success"){
							//var serial = msg[0].message1;
							openDialog("重置密码","<%=path%>/inner/pwdmodify/gotoContractCancelPwdModify.action?contractID="+contractID,400,300);
						}else{
							alert(msg[0].message);
						}
					},
		...
		function doPwdSave(){
			if(!$("#frmInfo").form("validate")){
	 		...
	 		$.ajax({
					type : "POST",
					contentType : "application/x-www-form-urlencoded;charset=utf-8",
					url : "<%=path%>/inner/pwdmodify/contractCancelPwdModify.action",
		...
		function opeanWin(){
			var string1 = "合同代码："+$("#contractID").textbox('getValue');
			var string2 = "用户姓名："+$("#buyer").combobox('getText');
	...
	<body id="body-layout" class="easyui-layout" style="margin-right: 1px; margin-left: 1px;">
		<div id="p" class="easyui-panel" title="合同乙方查询" style="width: 100%; height: 100%; background: #fafafa;"
			...
			<form method="post"  action="">
				...
				<table style="text-align:center;margin:0px auto;">
					<tr>
						<td colspan="2" style="font-size:16px">输入合同代码：<input class="easyui-textbox" type="text" name="contractID" id="contractID" data-options="required:true"/></td>
					</tr>
					<tr>
