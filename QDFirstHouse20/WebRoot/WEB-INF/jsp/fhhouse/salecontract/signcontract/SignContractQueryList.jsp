...
<script type="text/javascript">
	...
	function doHouseView(obj1,obj2){
		...
		if(obj2==8){
			$('#status').val(obj2);
			var url="<%=contextRoot%>/outer/signcontract/contractSellerCheck.action?projectID="+projectID+"&houseId="+obj1;
			...
			$.ajax({
				...
				url : "<%=contextRoot%>/outer/signcontract/doQueryCheckOther.action",
				data : {"houseID":obj1},
				dataType : "json",
				success : function(data){
					if(data[0].result == "success"){//没有抵押
						//window.open("<%=contextRoot%>/outer/signcontract/doHouseView.action?projectID="+projectID+"&startID"+startID+"&houseId="+obj1);
						var url = "<%=contextRoot%>/outer/signcontract/doHouseView.action?projectID="+projectID+"&startID="+startID+"&houseId="+obj1+"&status="+obj2;
