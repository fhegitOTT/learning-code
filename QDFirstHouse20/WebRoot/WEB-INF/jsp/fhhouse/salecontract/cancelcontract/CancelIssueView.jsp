...
<html xmlns="http://www.w3.org/1999/xhtml">
	...
	<script type="text/javascript">
		...
		function goback(){
			window.location.href="<%=path%>/inner/contractmanage/cancelIssue.action";
		}
		...
		function getValue(){  
	 	...
	 	function doSave(){
	 		...
	 		if(a==1 || a==2){
	 			var url="<%=path%>/inner/contractmanage/doThirdSubmit.action?ID=${cvo.ID}&status=${cvo.status}&cID=${cvo.contractID}&option="+a;
		 		window.openDialog("合同撤销发布",url,600,200);
	 		}else{
	 			alert("请输入发布意见后再提交！");
	 			window.history.back;
	 		}
	...
	<table class="input_table" cellpadding="5" cellspacing="1px" style="width:100%;background-color: #7DD5FF">
		...
		<tr height="25">
			...
			<td class="input_right" width="30%">
				<input class="easyui-textbox" type="text" name="agentCode" id="agentCode"  data-options="required:false,validType:'length[0,30]',editable:false"  value="${cvo.area}" style="width: 150px"></input>
			</td>
			<td class="input_left" bgcolor="#5BADFF" width="15%"><font style="color: white">总价：</font></td>
			<td class="input_right">
				<input class="easyui-textbox" type="text" name="legalManCode" id="legalManCode"  data-options="required:false,validType:'length[0,30]',editable:false"  value="${cvo.cost}" style="width: 150px"></input>
			</td>
		</tr>
		<tr height="25">
			<td class="input_left" bgcolor="#5BADFF"><font style="color: white">申 请 人：</font></td>
			<td class="input_right">
				<input class="easyui-textbox" type="text" name="regadr" id="regadr"  data-options="required:false,validType:'length[0,50]',editable:false"  value="${cvo.proposer1}" style="width: 300px"></input>
			</td>
		...
		<tr height="25">
			...
			<td class="input_right">
				<input class="easyui-textbox" name="post" id="post" value="${cvo.proposer1CardCode}"
					data-options="required:false,validType:'length[0,30]',editable:false" style="width: 150px" />
			</td>
		</tr>
		<tr height="25">
		...
		<tr height="25">
			...
			<td class="input_right" colspan="3">
				<input class="easyui-textbox" type="text" name="name" id="name"  data-options="required:false,validType:'length[0,30]'"  value="${cvo.cancelComment}" style="width: 500px" readonly="readonly"></input>
			</td>
		</tr>
		<tr>
			<td class="input_left " bgcolor="#5BADFF"><font style="color: white">经办意见：</font></td>
        	<td class="input_right" colspan="3">
				<textarea rows="6" name="workScope" id="workScope" cols="70" class="easyui-validatebox" data-options="required:false,validType:'length[0,100]'" readonly="readonly">${cvo.firstNotion}</textarea>
			</td> 
		</tr>
		<%
		...
		<tr height="25">
			...
			<td class="input_right">
				<input class="easyui-datebox" type="text" name="proxyPostcode" id="proxyPostcode"  data-options="required:false,validType:'length[0,30]',editable:false"  value="${cvo.firstDate1}" style="width: 100px"></input>
			</td>
		</tr>
		<tr>
			<td class="input_left " bgcolor="#5BADFF"><font style="color: white">复核意见：</font></td>
        	<td class="input_right" colspan="3">
				<textarea rows="4" name="firstMark" id="firstMark" cols="70" class="easyui-validatebox" data-options="required:false,validType:'length[0,100]'" readonly="readonly">${cvo.earlyNotion}</textarea>
			</td> 
		</tr>
		<tr height="25">
			...
			<td class="input_left" bgcolor="#5BADFF"><font style="color: white">日期：</font></td>
			<td class="input_right">
				<input class="easyui-datebox" type="text" name="proxyPostcode" id="proxyPostcode"  data-options="required:false,validType:'length[0,30]',editable:false"  value="${cvo.earlyDate1}" style="width: 100px"></input>
			</td>
		</tr>
		<c:if test="${cvo.status==6}">
		<tr height="25">
			<td class="input_left " bgcolor="#5BADFF"><font style="color: white">撤销原因：</font></td>
			<td class="input_right" colspan="3">
				<input class="easyui-combobox" name="reasonType" id="reasonType" value="${cvo.reasonType }"
					data-options="url:'<%=path%>/system/getDictionaryJson.action?dictName=CT_551',valueField:'code',textField:'name',
					multiple:false,editable:false,required:true,formatter: formatNullForCombobox" />
		</tr>
		<tr>
			<td class="input_left " bgcolor="#5BADFF"><font style="color: white">终审意见：</font></td>
        	<td class="input_right" colspan="3">
				<textarea rows="4" name="firstMark" id="firstMark" cols="70" class="easyui-validatebox" data-options="required:false,validType:'length[0,100]'" readonly="readonly">${cvo.finalNotion}</textarea>
			</td> 
		</tr>
		<tr height="25">
			<td class="input_left" bgcolor="#5BADFF"><font style="color: white">终审人：</font></td>
			<td class="input_right" >
			...
			<td class="input_left" bgcolor="#5BADFF"><font style="color: white">终审日期：</font></td>
			<td class="input_right">
		...
		<tr>
			<td class="input_left input_required" bgcolor="#5BADFF"><font style="color: white">是否发布：</font></td>
        	<td class="input_right" colspan="3">
				<input name="status" type="radio" value="1" onclick="getValue()"/>同意&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
				<input name="status" type="radio" value="2" onclick="getValue()"/>不同意  
			</td> 
		</tr>
		</c:if>

		<c:if test="${cvo.status==2 || cvo.status==5 || cvo.status==7}">
		<tr>
			<td class="input_left " bgcolor="#5BADFF"><font style="color: white">审批意见：</font></td>
        	<td class="input_right" colspan="3">
				<textarea rows="4" name="firstMark" id="firstMark" cols="70" class="easyui-validatebox" data-options="required:true,validType:'length[0,100]',editable:false">${cvo.finalNotion}</textarea>
			</td> 
		</tr>
		<tr height="25">
