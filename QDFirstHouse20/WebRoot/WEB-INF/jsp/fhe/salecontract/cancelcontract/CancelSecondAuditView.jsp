...
<html xmlns="http://www.w3.org/1999/xhtml">
	...
	<script type="text/javascript">
		function doBack(){
			window.location.href="<%=path%>/inner/contractmanage/cancelSecondAudit.action";
		}
		...
		function getValue(){  
	 	...
	 	function doSave(){
	 		var c=$("textarea[name='finalMark']").val();
	 		c= encodeURI(c);
	 		var a=$("input[name='status']:checked").val();
	 		var b=$("input[name='reasonType']").val();
	 		if(a==1 || a==2){
	 			var url="<%=path%>/inner/contractmanage/doSecondSubmit.action?ID=${cvo.ID}&status=${cvo.status}&cID=${cvo.contractID}&option="+a+"&reasonType="+b+"&finalMark="+c;
		 		window.openDialog("合同撤销",url,600,200);
	 		}else{
	 			alert("请输入审批意见后再提交！");
	 			window.history.back;
	 		}
...
<body id="body-layout" >
	...
	<table class="input_table" cellpadding="5" cellspacing="1px" style="width:100%;background-color: #7DD5FF">
		...
		<c:if test="${cvo.status==4 || cvo.status==7}">
		<tr height="25">
			<td class="input_left " bgcolor="#5BADFF"><font style="color: white">撤销原因：</font></td>
			<td class="input_right" colspan="3">
				<input class="easyui-combobox" name="reasonType" id="reasonType" value="1"
		...
		<tr>
			<td class="input_left input_required" bgcolor="#5BADFF"><font style="color: white">请输入审批意见：</font></td>
        	<td class="input_right" colspan="3">
