...
Date:				2005-2-1			
---------------------------------------------*/
document.write("<div id='meizzCalendarLayer' style='position: absolute;LEFT: 0px;TOP: 0px;  z-index: 9999; width: 144; height: 193; display: none'>");
...
this.eventSrc   = null;                     //日历显示的触发控件
    ...
    this.iframe     = window.frames[0]; //日历的 iframe 载体window.frames("meizzCalendarIframe")
    ...
    this.darkColor  = "#008A00";    //控件的暗色
    this.lightColor = "#FFFFFF";    //控件的亮色
    this.btnBgColor = "#CDF1C7";    //控件的按钮背景色
    this.wordColor  = "#000040";    //控件的文字颜色
    this.wordDark   = "#B4B4B4";    //控件的暗文字颜色
    this.dayBgColor = "#CDF1C7";    //日期数字背景色
    this.todayColor = "#AAAA46";    //今天在日历上的标示背景色
...
function calendarDiv(scrollTop, scrollLeft, adjustValue) //主调函数
{
	callCalendar(scrollTop, scrollLeft, adjustValue);
}
function callCalendar(scrollTop, scrollLeft, adjustValue)
{
    var e = window.event.srcElement;   writeIframe();
    var o = WebCalendar.calendar.style; WebCalendar.eventSrc = e;

//update by gyh 2005-3-22 start
//	if (arguments.length == 0) WebCalendar.objExport = e;
//    else WebCalendar.objExport = eval(arguments[0]);
	WebCalendar.objExport = e;
//update by gyh 2005-3-22 end
	...
	//insert by gyh 2005-3-22 start
	var ss;
	var tt;
	ss = o.top;
	tt = ss.substr(0, ss.length-2);
	o.top = tt - adjustValue - scrollTop;
	//insert by gyh 2005-3-22 end
	...
	ss = o.left;
	tt = ss.substr(0, ss.length-2);
	o.left = tt - scrollLeft;
	//insert by gyh 2005-3-22 end

	try{
        if (WebCalendar.objExport.value.trim() != ""){
            WebCalendar.dateStyle = WebCalendar.objExport.value.trim().match(WebCalendar.dateReg);
			//alert("=====datestyle"+WebCalendar.dateStyle);
...
function writeCalendar() ////对日历显示的数据的处理程序
{
 ...
 //如果选择日期小于当前日期，显示×
 //if (currentD < strtoday)
 //a[0] = '×';
        WebCalendar.dayObj[i].innerText    = a[0];
         a = WebCalendar.day[i].split("/");
...
// {
//alert('查询日期不能早于当前日期'); 
//        var c = WebCalendar.format.match(/^(\w{4})(-|\/|.|)(\w{1,2})\2(\w{1,2})$/);
//returnValue = flag ? d.getFullYear() +c[2]+ appendZero(strMonth) +c[2]+ appendZero(strDay) : d.getFullYear() +c[2]+ strMonth +c[2]+ strDay;
// }
