package com.netcom.nkestate.fhhouse.query.bo;
...
public class ContractPdfBO extends MiniBO {
	 ...
	 * @throws Exception
	 */
	public ContractPdfVO createSellContractPdfFHE(long contractID,int userType,int signFlag) throws Exception {
		...
		try{
			...
			//查询合同主表信息
			ContractDealFHEVO cdvo = (ContractDealFHEVO) cqDao.find(ContractDealFHEVO.class, contractID);
			//出售合同信息
...
"四、房地产开发企业出售已抵押的商品房，应事先以书面形式通知抵押权人，同时，将已抵押的事实告知购房人。未通知抵押权人或未告知购房人的，转让行为无效。购房人在签约前可向市、区、市房地产交易中心查询拟购房屋是否已抵押以及是否存在被司法机关或行政机关依法裁定、决定查封或者其他形式限制房地产权利的情况。",
			...
			if(list2 != null && list2.size() > 0){
				for(BuyerInfoVO bvo : list2){
					...
					String buyerBirth = DateUtil.parseDateTime3(bvo.getBuyerBirth());
					document.add(getPTable(new String[] { "个人/公司", "*" + baseHandle(bvo.getAttribute("buyer_type_dict_name")), " 性别：", " *" + baseHandle(bvo.getAttribute("buyer_sex_dict_name")), " 出生年月日： ", " *" + baseHandle(buyerBirth) }, 15, 15, 9, 9, 18, 34));
...
new String[] { "住址： ", "   *" + baseHandle(bvo.getBuyerAddress()), " 邮编：", " *" + baseHandle(bvo.getBuyerPostcode()) }, 9, 67, 9,
			...
			if("1".equals(isNewVer)){
				document.add(getParagraph("^第一条 甲方依法取得   " + "  *" + scvo.getN1_1() + "   区／市  " + "  *" + scvo.getN1_2() + " 地块土地使用权（出让合同编号：  " + "  *" + scvo.getN1_3() + "  ），投资建造的《  " + "  *" + scvo.getN1_4() + "  》商品房已竣工，并办理了房屋初始登记。"));
			}else{
				document.add(getParagraph("^第一条 甲方依法取得   " + "  *" + scvo.getN1_1() + "   区／市  " + "  *" + scvo.getN1_2() + " 地块土地使用权（出让合同编号：  " + "  *" + scvo.getN1_3() + "  ），投资建造的《  " + "  *" + scvo.getN1_4() + "  》商品房已竣工，并取得了房地产权属登记证明，证书号：" + "  *" + scvo.getN1_5() + "。"));
			}
			document.add(getParagraph("^第二条 乙方向甲方购买   " + "  *" + scvo.getN1_1() + "  区／市  " + "  *" + scvo.getN2_1() + "  路  " + "  *" + scvo.getLaneName() + " 号《   " + "  *" + scvo.getN2_3() + "   》    " + "  *" + scvo.getSubLane() + "   栋    " + "  *" + scvo.getN2_4() + "  单元    " + "  *"
					+ scvo.getN2_5() + "   层    " + "  *" + scvo.getN2_6() + "   户（以下简称该房屋）。据   " + "  *" + scvo.getChargeDep() + "   认定的测绘机构测量该房屋建筑面积为    " + "  *" + scvo.getN2_8() + "  平方米， 其中套内建筑面积为    " + "  *" + scvo.getN2_9() + "   平方米、公用分摊建筑面积为    " + "  *" + scvo.getN2_10()
			...
			document.add(getParagraph("^第四条  乙方应按本合同约定的付款方式和付款期限将房价款解入甲方的帐户／监管帐户（金融机构／监管机构：   " + "  *" + scvo.getN4_1() + "     、帐户名称：    " + "  *" + scvo.getN4_2() + "     、帐号：    " + "  *" + scvo.getN4_3() + "     ）。 "));
			document.add(getParagraph("乙方的付款时间和付款方式由甲、乙双方在本合同附件一中约定明确。"));
			document.add(getParagraph("甲方收到乙方支付的每一笔房款时均应开具发票。 "));
			document.add(getParagraph("^第五条 乙方如未按合同约定的时间付款，应当向甲方支付违约金，违约金按逾期 未付款的日万分之    " + "  *" + scvo.getN5_1() + "   计算，违约金自本合同的应付款期限之第二天起算至实际 付款之日止。逾期超过    " + "  *" + scvo.getN5_2() + "   天，甲方有权选择下列第    " + "  *" + getRes(scvo.getN5_3(), 2) + "  种方案追究乙方责任： "));
			...
			document.add(getParagraph("^第九条   乙方应在本合同第七条约定的房屋交接期限内到甲方要求的地点办理验收交接手续。该房屋交付的标志为      " + "  *" + scvo.getN9_1() + "     。该房屋交付之日起，该房屋的风险责任由 甲方转移给乙方。"));
			document.add(getParagraph("该房屋为        " + "  *" + scvo.getN9_2() + "    用房，甲方应向乙方提供《      " + "  *" + scvo.getN9_3() + "     质量保证书》和《     " + "  *" + scvo.getN9_4() + "    使用说明书》《前期物业管理服务合同》／《       " + "  *" + scvo.getN9_5() + "      》。乙方要求提供房屋实测面积的有关资料，甲方应如实提供。"));
			...
			document.add(getParagraph("如本合同约定的付款时间（附件一）的最后付款日期迟于本合同第七条约定的房屋交付日期的（即乙方未支付完全部房价款时），除甲方原因外，乙方未按约定期限办理交付该房屋手续的，甲方应当发出书面催告书一次。乙方未按催告书规定的日期办理该房屋的验收交接手续的，甲方有权单方面解除本合同。 "));
			document.add(getParagraph("^第十二条   甲方行使本合同条款中约定的单方面解除本合同权利时，应书面通知乙方。在书面通知发出之日起      " + "  *" + scvo.getN12_1() + "     日内将乙方已支付的房价款扣除乙方应支付的赔偿金额，剩余房价款退还乙方。赔偿金额以总房价款的      " + "  *" + scvo.getN12_2() + "    ％计算。"));
			document.add(getParagraph("^第十三条   除不可抗力外，甲方如未在本合同第七条约定的期内将该房屋交付乙方使用，应当向乙方支付违约金，违约金按乙方已支付的房价款日万分之      " + "  *" + scvo.getN13_1() + "   计算，违约金自本合同第七条约定的最后交付期限之第二天起算至实际交付之日止。逾期超过       " + "  *" + scvo.getN13_2() + "   天，乙方有权选择下列第      " + "  *" + getRes(scvo.getN13_3(), 2)
					+ "    种方案追究甲方责任："));
			document.add(getParagraph("一、 乙方有权单方面解除本合同。"));
			document.add(getParagraph("二、     " + "  *" + sellN13_4 + " "));
			document.add(getParagraph("^第十四条   自该房屋正式交付之时起，甲方对该房屋负责保修，并从房地产权利转移之日起继续保修二年／      " + "  *" + scvo.getN14_1() + "     年。保修范围由甲、乙双方参照国务院发布的《建设工程质量管理条例》规定在本合同附件五中约定。"));
			...
			document
.add(getParagraph("^第十九条    甲、乙双方商定，    " + "  *" + scvo.getN19_1() + "     年     " + "  *" + scvo.getN19_2() + "    月      " + "  *" + scvo.getN19_3() + "    日前，由      " + "  *" + dicBo.getDictName("ct_511", String.valueOf(scvo.getRegType())) + "    向      " + "  *"
					+ dicBo.getDictName("ct_510", String.valueOf(scvo.getN19_4())) + "   办理价格申报及过户申请手续，申领该房屋房地产权证。"));
			document.add(getParagraph("因甲方原因，乙方无法在     " + "  *" + scvo.getN19_5() + "  年     " + "  *" + scvo.getN19_6() + "  月      " + "  *" + scvo.getN19_7() + "    日前取得房地产权证，甲方应承担违约责任，违约金为总房价款的      " + "  *" + scvo.getN19_8() + "    ％；    " + "  *" + scvo.getN19_9() + "    年     " + "  *"
					+ scvo.getN19_10() + "  月     " + "  *" + scvo.getN19_11() + "     日之日起的       " + "  *" + scvo.getN19_12() + "    日内，乙方仍无法取得房地产权证，则乙方有权单方面解除合同。"));
			document.add(getParagraph("^第二十条     乙方行使本合同条款中约定的单方面解除本合同权利时，应书面通知甲方，甲方应当在收到乙方的书面通知之日起     " + "  *" + scvo.getN20_1() + "  天内将乙方已支付的房价款（包括利息 ，利息按中国人民银行公布的同期存款利率计算，下同）全部退还乙方，并承担赔偿责 任，赔偿金额以总房价款的     " + "  *" + scvo.getN20_2() + "  ％计算，在退还房价款时一并支付给乙方。 "));
			...
			document.add(getParagraph("^第二十二条    甲方或乙方对相对方行使单方面解除本合同有异议的，应在接到对方有关单方面解除本合同的书面通知之日起     " + "  *" + scvo.getN22_1() + "   天内，向按第二十七条选定的解决争议机关确认解除合同的效力。"));
			document.add(getParagraph("^第二十三条    甲方出售的该房屋仅作  " + "  *" + scvo.getN23_1() + "  使用，乙方在使用期间不得擅自改变该房屋结构和用途。除本合同附件另有约定外，乙方在使用期间有权享用与该房屋有关联的公共通道和设施，同时应自觉遵守该房屋所在地的有关法律、法规和尊重社会公德、维护公共设施和公共利益。"));
			document.add(getParagraph("^第二十四条    乙方购买的房屋及其相应占有的土地使用权不可分离。自该房屋的房地产权利转移之时起，甲方与       " + "  *" + scvo.getN24_1() + "  签订的土地使用权      " + "  *" + dicBo.getDictName("ct_507", String.valueOf(scvo.getLandpactType())) + "   合同中约定的权利、义务和责任依法转移给乙方。"));
			...
			document.add(getParagraph("^第二十七条    本合同适用中华人民共和国法律。甲、乙双方在履行本合同过程中发生争议，应协商解决。协商不能解决的，选定下列第     " + "  *" + getRes(scvo.getResolveType(), 2) + "   种方式解决："));
			...
			document.add(getParagraph("^第二十九条     本合同壹式    " + "  *" + scvo.getN29_1() + "   份，均具有同等效力。其中甲、乙双方各执    " + "  *" + scvo.getN29_2() + "   份，     " + "  *" + scvo.getN29_3() + "    、       " + "  *" + scvo.getN29_4() + "  、      " + "  *" + scvo.getN29_5() + "   各执壹份。"));
			...
			if(ptmr != null && ptmr.size() > 0){
				if(a1mVOList != null && a1mVOList.size() > 0){
					...
					for(PresellAttach2MoneyVO a1mVO : a1mVOList){
						...
						if(a1mVO.getState() == 1){
							document.add(getParagraph("付款方式：    " + " *" + a1mVO.getPaymentMode() + "    贷款方式：" + " *" + str1 + " 。", setChinese(FS10), 0.0f, 20.0f, 0.0f, Element.ALIGN_LEFT));
							document.add(getParagraph("（一）基本条款：1、乙方按以下方式向甲方支付本合同第三条约定的合同总价款     " + " *" + a1mVO.getTotalMoney() + "  元。", setChinese(FS10), 0.0f, 20.0f, 0.0f, Element.ALIGN_LEFT));
							document.add(getParagraph(" 乙方向甲方于   " + substringDate(String.valueOf(a1mVO.getFkDate()), 1) + " 年 " + substringDate(String.valueOf(a1mVO.getFkDate()), 2) + " 月 " + substringDate(String.valueOf(a1mVO.getFkDate()), 3) + " 日前一次性支付总价款 " + " *" + a1mVO.getMoney()
									+ " 元（含定金    " + "  *" + a1mVO.getDeposit() + " 元），存入本合同指定的账户。 ", setChinese(FS10), 0.0f, 20.0f, 0.0f, Element.ALIGN_LEFT));
						}
						if(a1mVO.getState() == 2 && a1mVO.getStateSign() == 0){
							document.add(getParagraph("付款方式：    " + "  *" + a1mVO.getPaymentMode() + "    贷款方式：    " + " *" + a1mVO.getBorrowMode() + " 。", setChinese(FS10), 0.0f, 20.0f, 0.0f, Element.ALIGN_LEFT));
							document.add(getParagraph("（一）基本条款：1、乙方按以下方式向甲方支付本合同第三条约定的合同总价款     " + " *" + a1mVO.getTotalMoney() + "  元。", setChinese(FS10), 0.0f, 20.0f, 0.0f, Element.ALIGN_LEFT));
							document.add(getParagraph(
									"首付款：    " + "  *" + a1mVO.getMoney() + " 元（含定金    " + "  *" + a1mVO.getDeposit() + " 元），乙方应与 " + substringDate(String.valueOf(a1mVO.getFkDate()), 1) + " 年 " + substringDate(String.valueOf(a1mVO.getFkDate()), 2) + " 月 "
											+ substringDate(String.valueOf(a1mVO.getFkDate()), 3) + " 日前存入本合同指定的账户。", setChinese(FS10), 0.0f, 20.0f, 0.0f, Element.ALIGN_LEFT));
							...
							if(list3.size() == 1){
								document.add(getParagraph(
										"剩余房价款:    " + " *" + list3.get(0).getMoney() + "  元，乙方应与  " + substringDate(String.valueOf(list3.get(0).getFkDate()), 1) + " 年 " + substringDate(String.valueOf(list3.get(0).getFkDate()), 2) + " 月 " + substringDate(String.valueOf(list3.get(0).getFkDate()), 3)
												+ " 日前存入本合同指定的账户。", setChinese(FS10), 0.0f, 20.0f, 0.0f, Element.ALIGN_LEFT));
							}else{
								for(int i = 0; i < list3.size(); i++){
									document.add(getParagraph(
											"首付款" + " *" + (i + 1) + " 期：  " + " *" + list3.get(i).getMoney() + "  元，乙方应与  " + substringDate(String.valueOf(list3.get(i).getFkDate()), 1) + " 年 " + substringDate(String.valueOf(list3.get(i).getFkDate()), 2) + " 月 "
													+ substringDate(String.valueOf(list3.get(i).getFkDate()), 3) + " 日前存入本合同指定的账户。", setChinese(FS10), 0.0f, 20.0f, 0.0f, Element.ALIGN_LEFT));
						...
						if(a1mVO.getState() == 3 && a1mVO.getStateSign() == 0){
							...
							if(list3.size() == 1){
								document.add(getParagraph(
										"剩余房价款:    " + "  *" + list3.get(0).getMoney() + "  元，乙方应与  " + substringDate(String.valueOf(list3.get(0).getFkDate()), 1) + " 年 " + substringDate(String.valueOf(list3.get(0).getFkDate()), 2) + " 月 "
												+ substringDate(String.valueOf(list3.get(0).getFkDate()), 3) + " 日前存入本合同指定的账户。", setChinese(FS10), 0.0f, 20.0f, 0.0f, Element.ALIGN_LEFT));
							}else{
								for(int i = 0; i < list3.size(); i++){
									document.add(getParagraph(
											"首付款" + " *" + (i + 1) + " 期：  " + " *" + list3.get(i).getMoney() + "  元，乙方应与  " + substringDate(String.valueOf(list3.get(i).getFkDate()), 1) + " 年 " + substringDate(String.valueOf(list3.get(i).getFkDate()), 2) + " 月 "
						...
						if(a1mVO.getState() == 4){
							document.add(getParagraph(
									"贷款：    " + "  *" + a1mVO.getMoney() + " 元，乙方以 " + " *" + a1mVO.getBorrowMode() + " 方式应于 " + substringDate(String.valueOf(a1mVO.getFkDate()), 1) + " 年 " + substringDate(String.valueOf(a1mVO.getFkDate()), 2) + " 月 "
											+ substringDate(String.valueOf(a1mVO.getFkDate()), 3) + " 日前将办理按揭贷款所需文件全部提交 " + " *" + a1mVO.getBankName() + " ，放贷机构于 " + substringDate(String.valueOf(a1mVO.getDkDate()), 1) + " 年 " + substringDate(String.valueOf(a1mVO.getDkDate()), 2) + " 月 "
											+ substringDate(String.valueOf(a1mVO.getDkDate()), 3) + " 日前将贷款存入本合同指定的账户。 ", setChinese(FS10), 0.0f, 20.0f, 0.0f, Element.ALIGN_LEFT));
						}
					}
				...
				if(attach3vo != null){
					...
					if(attach4vo != null && attach4vo.size() > 0){
						for(Attach1VO att4VO : attach4vo){
							...
							if(att4VO.getPaymentType() == 0){
								...
								document.add(getParagraph("付款方式： " + " *" + paymentMode + "  贷款方式： " + " *" + borrowMode, setChinese(FS10), 0.0f, 20.0f, 0.0f, Element.ALIGN_LEFT));
								document.add(getParagraph(
										"1、乙方于  " + substringDate(String.valueOf(attach3vo.getPayDate()), 1) + " 年 " + substringDate(String.valueOf(attach3vo.getPayDate()), 2) + " 月 " + substringDate(String.valueOf(attach3vo.getPayDate()), 3) + " 日前付定金计人民币" + " *" + attach3vo.getDeposit()
												+ " 元；（大写） " + " *" + attach3vo.getDepositCn() + " 。", setChinese(FS10), 0.0f, 20.0f, 0.0f, Element.ALIGN_LEFT));
								...
								if(attach4vo != null && attach4vo.size() > 0){
									for(Attach1MoneyVO Atmyvo : atmyvo){
										if(Atmyvo.getSerial() == 1){
											document.add(getParagraph("2、乙方于   " + substringDate(String.valueOf(Atmyvo.getPaymentDate()), 1) + " 年 " + substringDate(String.valueOf(Atmyvo.getPaymentDate()), 2) + " 月 " + substringDate(String.valueOf(Atmyvo.getPaymentDate()), 3)
													+ " 日与甲方签约，并付首付房款计人民币 " + " *" + Atmyvo.getMoney() + " 元；（大写） " + " *" + Atmyvo.getMoneyCn() + " 。", setChinese(FS10), 0.0f, 20.0f, 0.0f, Element.ALIGN_LEFT));

										}else{
											document.add(getParagraph("2、乙方于   " + substringDate(String.valueOf(Atmyvo.getPaymentDate()), 1) + " 年 " + substringDate(String.valueOf(Atmyvo.getPaymentDate()), 2) + " 月 " + substringDate(String.valueOf(Atmyvo.getPaymentDate()), 3) + " 日前应支付房款计人民币"
													+ " *" + Atmyvo.getMoney() + " 元；（大写） " + " *" + Atmyvo.getMoneyCn() + " 。", setChinese(FS10), 0.0f, 20.0f, 0.0f, Element.ALIGN_LEFT));
										}
									}
								}
					...
					if(attach4vo != null && attach4vo.size() > 0){
						for(Attach1VO att4VO : attach4vo){
							...
							if(att4VO.getPaymentType() == 0){
								document.add(getParagraph("按时间付款   ", setChinese(FS10), 0.0f, 20.0f, 0.0f, Element.ALIGN_LEFT));
								document.add(getParagraph("付款方式： " + " *" + att4VO.getAttribute("paymentMode_dict_name") + "  贷款方式： " + " *" + att4VO.getAttribute("borrowMode_dict_name"), setChinese(FS10), 0.0f, 20.0f, 0.0f, Element.ALIGN_LEFT));
								...
								if(attach4vo != null && attach4vo.size() > 0){
									for(Attach1MoneyVO Atmyvo : atmyvo){
										if(Atmyvo.getSerial() == 1){
											document.add(getParagraph("1、乙方于   " + substringDate(String.valueOf(Atmyvo.getPaymentDate()), 1) + " 年 " + substringDate(String.valueOf(Atmyvo.getPaymentDate()), 2) + " 月 " + substringDate(String.valueOf(Atmyvo.getPaymentDate()), 3)
													+ " 日与甲方签约，并付首付房款计人民币 " + " *" + Atmyvo.getMoney() + " 元；（大写） " + " *" + Atmyvo.getMoneyCn() + " 。", setChinese(FS10), 0.0f, 20.0f, 0.0f, Element.ALIGN_LEFT));

										}else{
											document.add(getParagraph("  1、乙方于   " + substringDate(String.valueOf(Atmyvo.getPaymentDate()), 1) + " 年 " + substringDate(String.valueOf(Atmyvo.getPaymentDate()), 2) + " 月 " + substringDate(String.valueOf(Atmyvo.getPaymentDate()), 3) + " 日前应支付房款计人民币"
													+ " *" + Atmyvo.getMoney() + " 元；（大写） " + " *" + Atmyvo.getMoneyCn() + " 。", setChinese(FS10), 0.0f, 20.0f, 0.0f, Element.ALIGN_LEFT));
										}
									}
								}
	...
	public ContractPdfVO createSellContractPdf(long contractID,int userType,int signFlag) throws Exception {
		...
		try{
			...
			//页眉
			HeaderFooter header = new HeaderFooter(new Phrase(String.valueOf(contractID) + "---------------------------------------------------------------------------------------------"
					+ DateUtil.formatDateTime(DateUtil.parseDateTime2( (DateUtil.getSysDateYYYYMMDD() + DateUtil.getSysDateHHMMSS()))), setChinese(FS10)), false);
			header.setBorder(Rectangle.NO_BORDER);
			header.setAlignment(1);
			header.setBorderColor(Color.BLACK);
			document.setHeader(header);
			...
			if("0".equals(isNewVer)){
				document.add(getParagraph("一、本合同文本是根据有关房地产转让的法律、法规、规定制定的示范文本，印制的合同条款为提示性条款，供未经预售阶段的商品房（现房）出售时当事人双方约定采用。", setChinese(FS14), FL28, 20.0f, 0.0f, Element.ALIGN_LEFT));
				document.add(getParagraph("二、购房是一种民事法律行为，涉及的标的额较大、专业性较强、法律规范较多，为更好地维护双方当事人的权益，双方签订合同时应当慎重，力求签订得具体、全面、严密。", setChinese(FS14), FL28, 20.0f, 0.0f, Element.ALIGN_LEFT));
				document.add(getParagraph("三、商品房在取得房地产权属登记证明后，方可出售。在签订出售合同前，房地产开发企业应向购房人出示房地产权属登记证明。新建商品房房地产权属登记证明真实性、合法性可向市、区、市房地产交易中心查询。", setChinese(FS14), FL28, 20.0f, 0.0f, Element.ALIGN_LEFT));
				document.add(getParagraph("四、房地产开发企业出售已抵押的商品房，应事先以书面形式通知抵押权人，同时，将已抵押的事实告知购房人。未通知抵押权人或未告知购房人的，转让行为无效。购房人在签约前可向市、区、市房地产交易中心查询拟购房屋是否已抵押以及是否存在被司法机关或行政机关依法裁定、决定查封或者其他形式限制房地产权利的情况。", setChinese(FS14), FL28, 20.0f, 0.0f, Element.ALIGN_LEFT));
				document.add(getParagraph("五、商品房出售合同签订后应及时向房地产登记机构办理商品房变更登记手续。购房人领取房地产权证。", setChinese(FS14), FL28, 20.0f, 0.0f, Element.ALIGN_LEFT));
				document.add(getParagraph("六、双方履行合同发生争议的，可选择向不动产所在地人民法院起诉，也可选择向仲裁委员会申请仲裁。如选择申请仲裁的，可向本市的仲裁委员会申请，也可向外地的仲裁委员会申请，本市仲裁委员会全称为青岛仲裁委员会。", setChinese(FS14), FL28, 20.0f, 0.0f, Element.ALIGN_LEFT));
			}
			if("1".equals(isNewVer)){
				document.add(getParagraph("一、本合同文本是根据有关房地产转让的法律、法规、规定制定的示范文本，印制的合同条款为提示性条款，供未经预售阶段的商品房（现房）出售时当事人双方约定采用。", setChinese(FS14), FL28, 20.0f, 0.0f, Element.ALIGN_LEFT));
				document.add(getParagraph("二、购房是一种民事法律行为，涉及的标的额较大、专业性较强、法律规范较多，为更好地维护双方当事人的权益，双方签订合同时应当慎重，力求签订得具体、全面、严密。", setChinese(FS14), FL28, 20.0f, 0.0f, Element.ALIGN_LEFT));
				document.add(getParagraph("三、房地产开发企业出售已抵押的商品房，应事先以书面形式通知抵押权人，同时，将已抵押的事实告知购房人。未通知抵押权人或未告知购房人的，转让行为无效。购房人在签约前可向市、区、市房地产交易中心查询拟购房屋是否已抵押以及是否存在被司法机关或行政机关依法裁定、决定查封或者其他形式限制房地产权利的情况。", setChinese(FS14), FL28, 20.0f, 0.0f, Element.ALIGN_LEFT));
				...
				document.add(getParagraph("五、双方履行合同发生争议的，可选择向不动产所在地人民法院起诉，也可选择向仲裁委员会申请仲裁。如选择申请仲裁的，可向本市的仲裁委员会申请，也可向外地的仲裁委员会申请，本市仲裁委员会全称为青岛仲裁委员会。", setChinese(FS14), FL28, 20.0f, 0.0f, Element.ALIGN_LEFT));
			}
			...
			if(list2 != null && list2.size() > 0){
				for(BuyerInfoVO bvo : list2){
					...
					buyername += bvo.getBuyerName();
					document.add(getPTable(new String[] { "乙方(买方)：", " *" + bvo.getBuyerName() }, 18, 82, 0, 0, 0, 0));
					document.add(getPTable(new String[] { "国籍： ", " *" + dBo.getDictName("ct_504", String.valueOf(bvo.getBuyerNationality())), "  居住(注册)所在省市：", " *" + dBo.getDictName("ct_524", String.valueOf(bvo.getBuyerProvince())) }, 9, 30, 30, 31, 0, 0));

					String buyerBirth = DateUtil.parseDateTime3(bvo.getBuyerBirth());
					document.add(getPTable(new String[] { "个人/公司", "*" + baseHandle(bvo.getAttribute("buyer_type_dict_name")), " 性别：", " *" + baseHandle(bvo.getAttribute("buyer_sex_dict_name")), " 出生年月日： ", " *" + baseHandle(buyerBirth) }, 15, 15, 9, 9, 18, 34));
					document.add(getPTable(new String[] { "住址： ", "   *" + baseHandle(bvo.getBuyerAddress()), " 邮编：", " *" + baseHandle(bvo.getBuyerPostcode()) }, 9, 67, 9, 15, 0, 0));
					document.add(getPTable(new String[] { "证件名称：", "  *" + dBo.getDictName("ct_506", String.valueOf(bvo.getBuyerCardname())), " 号码：", " *" + baseHandle(bvo.getBuyerCardcode()) }, 15, 40, 9, 36, 0, 0));
					document.add(getPTable(new String[] { "联系电话：", " *" + bvo.getBuyerCall() }, 15, 85, 0, 0, 0, 0));
					document.add(getPTable(new String[] { "委托/法定代理人：", "  *" + baseHandle(bvo.getBuyerProxy()), " 联系电话：", "  *" + baseHandle(bvo.getBuyerProxyCall()) }, 27, 33, 15, 25, 0, 0));
					document.add(getPTable(new String[] { "住址：", "   *" + baseHandle(bvo.getBuyerProxyAdr()) }, 12, 88, 0, 0, 0, 0));
				}
			}
				...
				if(a1mVOList != null && a1mVOList.size() > 0){
					...
					for(PresellAttach2MoneyVO a1mVO : a1mVOList){
						...
						if(a1mVO.getState() == 1){
							...
							document.add(getParagraph(" 乙方向甲方于   " + substringDate(String.valueOf(a1mVO.getFkDate()), 1) + " 年 " + substringDate(String.valueOf(a1mVO.getFkDate()), 2) + " 月 " + substringDate(String.valueOf(a1mVO.getFkDate()), 3) + " 日前一次性支付总价款 " + " *" + a1mVO.getMoney()
									+ " 元（含定金    " + "  *" + a1mVO.getDeposit() + " 元），存入本合同指定的账户。 ",
						...
						if(a1mVO.getState() == 2 && a1mVO.getStateSign() == 0){
							...
							if(list3.size() == 1){
								document.add(getParagraph(
										"剩余房价款:    " + " *" + list3.get(0).getMoney() + "  元，乙方应与  " + substringDate(String.valueOf(list3.get(0).getFkDate()), 1) + " 年 " + substringDate(String.valueOf(list3.get(0).getFkDate()), 2) + " 月 " + substringDate(String.valueOf(list3.get(0).getFkDate()), 3)
												+ " 日前存入本合同指定的账户。", setChinese(FS10), 0.0f, 20.0f, 0.0f,
										Element.ALIGN_LEFT));
							}else{
								for(int i = 0; i < list3.size(); i++){
									document.add(getParagraph(
							...
							if(att4VO.getPaymentType() == 0){
								...
								if(attach4vo != null && attach4vo.size() > 0){
									for(Attach1MoneyVO Atmyvo : atmyvo){
										if(Atmyvo.getSerial() == 1){
											document.add(getParagraph("2、乙方于   " + substringDate(String.valueOf(Atmyvo.getPaymentDate()), 1) + " 年 " + substringDate(String.valueOf(Atmyvo.getPaymentDate()), 2) + " 月 " + substringDate(String.valueOf(Atmyvo.getPaymentDate()), 3)
													+ " 日与甲方签约，并付首付房款计人民币 " + " *" + Atmyvo.getMoney() + " 元；（大写） " + " *" + Atmyvo.getMoneyCn() + " 。", setChinese(FS10), 0.0f, 20.0f, 0.0f,
													Element.ALIGN_LEFT));
...
"出卖人向买受人出售其开发建设的房屋，双方当事人应当在自愿、平等、公平及 诚实信用的基础上，根据《中华人民共和国合同法》、《中华人民共和国物权法》、《中 华人民共和国城市房地产管理法》等法律、法规的规定，就商品房买卖相关内容协商 达成一致意见，签订本商品房买卖合同。",
			...
			document.add(getParagraph(" 1. 出卖人以       " + " *" + cdvo.getf0101() + "    方式取得坐落于         " + " *" + cdvo.getf0102() + "      地块的建设用地使用权。该地块        " + " *" + cdvo.getf0103() + "     为      " + " *" + cdvo.getf0104() + "    土地使用权面积为      " + " *" + cdvo.getf0105()
					+ "   平方米。买受人购买的商品房（以下简称该商品 房）所占用的土地用途为    " + " *" + cdvo.getf0106() + " ，土地使用权终止日期为  " + " *" + cdvo.getf0107() + " 。",
					setChinese(FS14), SPB28, 25.0f, 0.0f, Element.ALIGN_LEFT));
			document.add(getParagraph("2. 出卖人经批准，在上述地块上建设的商品房项目核准名称为 " + " *" + cdvo.getf0108() + " ，建设工程规划许可证号为  " + " *" + cdvo.getf0109() + "     ，建筑工程施工许可证号为       " + " *" + cdvo.getf0110() + " 。", setChinese(FS14), SPB28, 25.0f, 0.0f, Element.ALIGN_LEFT));
			...
			document.add(getParagraph(" 2. 该商品房所在建筑物的主体结构为" + " *" + cdvo.getf0302() + " ，建筑总层 数为" + " *" + cdvo.getf0303() + " 层，其中地上 " + " *" + cdvo.getf0304() + " 层，地下 " + " *" + cdvo.getf0305() + " 层。", setChinese(FS14), SPB28, 25.0f, 0.0f, Element.ALIGN_LEFT));
			document.add(getParagraph(" 3. 该商品房为第一条规定项目中的" + " *" + cdvo.getf0306() + " 。房屋竣工后，如房号发生改变，不影响该商品房的特定位置。该商品房的平面图见附件一。", setChinese(FS14), SPB28,
					25.0f, 0.0f, Element.ALIGN_LEFT));
			document.add(getParagraph(" 4. 该商品房的房产测绘机构为 " + " *" + cdvo.getf0307() + " ,其预测建筑面积共" + " *" + cdvo.getf0308() + " 平方米,其中套内建筑面积  " + " *" + cdvo.getf0309() + " 平方米，分摊共有建筑 面积  " + " *" + cdvo.getf0310() + " 平方米。该商品房共用部位见附件二。 ", setChinese(FS14), SPB28, 25.0f, 0.0f,
					Element.ALIGN_LEFT));
			document.add(getParagraph("该商品房层高为 " + " *" + cdvo.getf0311() + " 米，有  " + " *" + cdvo.getf0312() + " 个阳台，其中  " + " *" + cdvo.getf0313() + " 个阳台为封 闭式，" + " *" + cdvo.getf0314() + " 个阳台为非封闭式。阳台是否封闭以规划设计文件为准。", setChinese(FS14), SPB28, 25.0f, 0.0f, Element.ALIGN_LEFT));
			...
			document.add(getParagraph(
" 如该商品房权利状况与上述情况不符，导致不能完成本合同登记备案或房屋所有 权转移登记的，买受人有权解除合同。买受人解除合同的，应当书面通知出卖人。出 卖人应当自解除合同通知送达之日起15日内退还买受人已付全部房款（含已付贷款部 分），并自买受人付款之日起，按照" + " *" + cdvo.getf0503() + " %（不低于中国人民银行公布的同期贷款基 准利率）计算给付利息。给买受人造成损失的，由出卖人支付  " + " *" + cdvo.getf0504() + " 的赔偿金。",
					setChinese(FS14), SPB28, 25.0f, 0.0f, Element.ALIGN_LEFT));
			...
			document.add(getParagraph(" 1. 按照套内建筑面积计算，该商品房单价（不包含房屋装修）为每平方米  " + " *" + cdvo.getf0615() + " " + "（币种）" + " *" + cdvo.getf0602() + " 元， 总价款（不包含房屋装修）为 " + " *" + cdvo.getf0616() + " " + "（币种）" + " *" + cdvo.getf0603() + " 元（大写  " + " *" + cdvo.getf0604() + " 元整）。该商品房装修价格为每平方米  " + " *"
					+ cdvo.getf0617() + " " + "（币种）" + " *" + cdvo.getf0618() + " 元， 装修总价为人民币 " + " *" + cdvo.getf0619() + " 元（大写 " + " *" + cdvo.getf0626() + " 元整）。", setChinese(FS14), SPB28, 25.0f, 0.0f, Element.ALIGN_LEFT));
			document.add(getParagraph(" 2. 按照建筑面积计算，该商品房单价（不包含房屋装修）为每平方米 " + " *" + cdvo.getf0620() + " " + "（币种）" + " *" + cdvo.getf0605() + " 元， 总价款（不包含房屋装修）为 " + " *" + cdvo.getf0621() + " " + "（币种）" + " *" + cdvo.getf0606() + " 元（大写  " + " *" + cdvo.getf0607() + " 元整）。该商品房装修价格为每平方米  " + " *"
					+ cdvo.getf0622() + " " + "（币种）" + " *" + cdvo.getf0608() + " 元， 装修总价为人民币 " + " *" + cdvo.getf0609() + " 元（大写 " + " *" + cdvo.getf0627() + " 元整）。", setChinese(FS14), SPB28, 25.0f, 0.0f, Element.ALIGN_LEFT));
			document.add(getParagraph(" 3. 按照套计算，该商品房总价款（不包含房屋装修）为   " + " *" + cdvo.getf0623() + " " + "（币种）" + " *" + cdvo.getf0610() + "  元（大 写  " + " *" + cdvo.getf0611() + " 元整）， 装修总价为人民币 " + " *" + cdvo.getf0624() + " 元（大写 " + " *" + cdvo.getf0638() + " 元整）。 ", setChinese(FS14), SPB28, 25.0f,
					0.0f, Element.ALIGN_LEFT));
			document.add(getParagraph(" 4. 按照    " + " *" + cdvo.getf0612() + " 计算，该商品房总价款为  " + " *" + cdvo.getf0625() + " " + "（币种）" + " *" + cdvo.getf0613() + " 元 （大写  " + " *" + cdvo.getf0614() + " 元整）。", setChinese(FS14), SPB28, 25.0f, 0.0f, Element.ALIGN_LEFT));
			document.add(getParagraph(" 5. 按照套内建筑面积计算，该商品房单价为每平方米  " + " *" + cdvo.getf0628() + " " + "（币种）" + " *" + cdvo.getf0629() + " 元， 总价款为 " + " *" + cdvo.getf0630() + " " + "（币种）" + " *" + cdvo.getf0631() + " 元（大写  " + " *" + cdvo.getf0632() + " 元整）。", setChinese(FS14), SPB28, 25.0f, 0.0f,
					Element.ALIGN_LEFT));
			document.add(getParagraph(" 6. 按照建筑面积计算，该商品房单价为每平方米  " + " *" + cdvo.getf0633() + " " + "（币种）" + " *" + cdvo.getf0634() + " 元， 总价款为 " + " *" + cdvo.getf0635() + " " + "（币种）" + " *" + cdvo.getf0636() + " 元（大写  " + " *" + cdvo.getf0637() + " 元整）。", setChinese(FS14), SPB28, 25.0f, 0.0f,
					Element.ALIGN_LEFT));
			document.add(getParagraph("^第七条   付款方式及期限    "));
			document.add(getParagraph(" (一)签订本合同前，买受人已向出卖人支付定金 " + " *" + cdvo.getf0728() + "  （币种）    *" + cdvo.getf0701() + " 元（大写     *" + cdvo.getf0702() + "  ）,该定金于     *" + cdvo.getf0703() + " 时   *" + cdvo.getf0705() + " 商品房价款。", setChinese(FS14), SPB28, 25.0f, 0.0f,
					Element.ALIGN_LEFT));
			document.add(getParagraph(" (二)买受人采取下列第  " + " *" + cdvo.getf0707() + " 种方式付款：   ", setChinese(FS14), SPB28, 25.0f, 0.0f, Element.ALIGN_LEFT));
			document.add(getParagraph(" 1. 一次性付款。买受人应当在   " + substringDate(cdvo.getf0708(), 1) + " 年 " + substringDate(cdvo.getf0708(), 2) + " 月 " + substringDate(cdvo.getf0708(), 3) + " 日前支付该商品房全部价款。    ", setChinese(FS14), SPB28, 25.0f, 0.0f, Element.ALIGN_LEFT));
			document.add(getParagraph(" 2. 分期付款。买受人应当在    " + substringDate(cdvo.getf0709(), 1) + " 年 " + substringDate(cdvo.getf0709(), 2) + " 月 " + substringDate(cdvo.getf0709(), 3) + " 日前分     " + " *" + cdvo.getf0710() + " 期支付该商品 房全部价款，首期房价款 " + " *" + cdvo.getf0729() + " " + "（币种）" + " *"
			...
			document.add(getParagraph(" 3. 贷款方式付款： " + " *" + cdvo.getf0715() + " 。买受人应当于   " + " *" + cdvo.getf0717() + "  日前支付首期房价款 " + " *" + cdvo.getf0730() + " " + "（币种）" + " *" + cdvo.getf0718() + " 元（大写：  " + " *" + cdvo.getf0719() + " 元整），占全部房价款 " + " *" + cdvo.getf0720()
					+ " % ",
					setChinese(FS14),
					SPB28, 25.0f, 0.0f, Element.ALIGN_LEFT));
			document.add(getParagraph(" 余款  " + " *" + cdvo.getf0731() + " " + "（币种）" + " *" + cdvo.getf0721() + " 元（大写  " + " *" + cdvo.getf0722() + "  元整)向    " + " *" + cdvo.getf0723() + " （贷款机构）申请贷款支付。  ", setChinese(FS14), SPB28, 25.0f, 0.0f, Element.ALIGN_LEFT));
			...
			document.add(getParagraph(" 该商品房的预售资金监管机构为  " + " *" + cdvo.getf0725() + " ，预售资金监管 账户名称为   " + " *" + cdvo.getf0726() + " ，账号为  " + " *" + cdvo.getf0727() + " 。", setChinese(FS14), SPB28, 25.0f, 0.0f, Element.ALIGN_LEFT));
			...
			document.add(getParagraph(" (1)逾期在  " + " *" + cdvo.getf0802() + " 日之内，买受人按日计算向出卖人支付逾期应付款万分之   " + " *" + cdvo.getf0803() + " 的 违约金。 ",
					setChinese(FS14), SPB28, 25.0f, 0.0f, Element.ALIGN_LEFT));
			document.add(getParagraph(" (2)逾期超过   " + " *" + cdvo.getf0804() + " 日（该期限应当与本条第（1）项中的期限相同）后，出卖人 有权解除合同。出卖人解除合同的，应当书面通知买受人。买受人应当自解除合同通 知送达之日起   " + " *" + cdvo.getf0805() + "  日内按照累计应付款的  " + " *" + cdvo.getf0806() + "  %向出卖人支付违约金，同时，出卖 人退还买受人已付全部房款（含已付贷款部分）。 ", setChinese(FS14),
					SPB28,
					25.0f, 0.0f, Element.ALIGN_LEFT));
			...
			document.add(getParagraph(" (1)以上设施中第1、2、3、4项在约定交付日未达到交付条件的，出卖人按照本 合同第十二条的约定承担逾期交付责任。     ", setChinese(FS14), SPB28, 25.0f, 0.0f,
					Element.ALIGN_LEFT));
			document.add(getParagraph(" 第5项未按时达到交付使用条件的，出卖人按日向买受人支付   " + " *" + cdvo.getf1006() + " 元的违约 金；第6项未按时达到交付使用条件的，出卖人按日向买受人支付   " + " *" + cdvo.getf1007() + "  元的违约 金；第7项未按时达到交付使用条件的，出卖人按日向买受人支付  " + " *" + cdvo.getf1008() + "  元的违约 金。出卖人采取措施保证相关设施于约定交付日后 " + " *" + cdvo.getf1009()
					+ " 日之内达到交付使用条件。", setChinese(FS14), SPB28, 25.0f, 0.0f, Element.ALIGN_LEFT));
			...
			document
.add(getParagraph(" 1. 小区内绿地率：   " + substringDate(cdvo.getf1011(), 1) + " 年 " + substringDate(cdvo.getf1011(), 2) + " 月 " + substringDate(cdvo.getf1011(), 3) + " 日达到    " + " *" + cdvo.getf1012() + " ； ", setChinese(FS14), SPB28, 25.0f, 0.0f,
							Element.ALIGN_LEFT));
			document.add(getParagraph(" 2. 小区内非市政道路：   " + substringDate(cdvo.getf1013(), 1) + " 年 " + substringDate(cdvo.getf1013(), 2) + " 月 " + substringDate(cdvo.getf1013(), 3) + "  日达到      " + " *" + cdvo.getf1014() + " ； ", setChinese(FS14), SPB28, 25.0f, 0.0f,
					Element.ALIGN_LEFT));
			document.add(getParagraph(" 3. 规划的车位、车库：  " + substringDate(cdvo.getf1015(), 1) + " 年 " + substringDate(cdvo.getf1015(), 2) + " 月 " + substringDate(cdvo.getf1015(), 3) + " 日达到     " + " *" + cdvo.getf1016() + " ； ", setChinese(FS14), SPB28, 25.0f, 0.0f,
					Element.ALIGN_LEFT));
			document.add(getParagraph(" 4.物业服务用房：  " + substringDate(cdvo.getf1017(), 1) + " 年 " + substringDate(cdvo.getf1017(), 2) + " 月 " + substringDate(cdvo.getf1017(), 3) + " 日达到     " + " *" + cdvo.getf1018() + " ； ", setChinese(FS14), SPB28, 25.0f, 0.0f,
					Element.ALIGN_LEFT));
			document.add(getParagraph(" 5. 医疗卫生机构：   " + substringDate(cdvo.getf1019(), 1) + " 年 " + substringDate(cdvo.getf1019(), 2) + " 月 " + substringDate(cdvo.getf1019(), 3) + "  日达到      " + " *" + cdvo.getf1020() + " ； ", setChinese(FS14), SPB28, 25.0f, 0.0f,
					Element.ALIGN_LEFT));
			document.add(getParagraph(" 6. 幼儿园：  " + substringDate(cdvo.getf1021(), 1) + " 年 " + substringDate(cdvo.getf1021(), 2) + " 月 " + substringDate(cdvo.getf1021(), 3) + "  日达到      " + " *" + cdvo.getf1022() + " ； ", setChinese(FS14), SPB28, 25.0f, 0.0f,
					Element.ALIGN_LEFT));
			document.add(getParagraph(" 7. 学校  " + substringDate(cdvo.getf1023(), 1) + " 年 " + substringDate(cdvo.getf1023(), 2) + " 月 " + substringDate(cdvo.getf1023(), 3) + "  日达到      " + " *" + cdvo.getf1024() + " ； ", setChinese(FS14), SPB28, 25.0f, 0.0f,
					Element.ALIGN_LEFT));
...
.add(getParagraph(" 3. 规划的车位、车库未达到上述约定条件的，   " + " *" + cdvo.getf1029() + " 。   ", setChinese(FS14), SPB28, 25.0f, 0.0f, Element.ALIGN_LEFT));
			...
			document.add(getParagraph(" (二)该商品房达到第九条、第十条约定的交付条件后,出卖人应当在交付日期届 满前     " + " *" + cdvo.getf1102() + "  日（不少于10日）将查验房屋的时间、办理交付手续的时间地点以及应当携 带的证件材料的通知书面送达买受人。买受人未收到交付通知书的,以本合同约定的 交付日期届满之日为办理交付手续的时间,以该商品房所在地为办理交付手续的地点。 ", setChinese(FS14),
					SPB28, 25.0f, 0.0f, Element.ALIGN_LEFT));
			...
			document.add(getParagraph("  1. 办理交付手续前，买受人有权对该商品房进行查验，出卖人不得以缴纳相关税 费或者签署物业管理文件作为买受人查验和办理交付手续的前提条件。  ", setChinese(FS14), SPB28, 25.0f, 0.0f,
					Element.ALIGN_LEFT));
			document.add(getParagraph("  2. 买受人查验的该商品房存在下列除地基基础和主体结构外的其他质量问题的， 由出卖人按照有关工程和产品质量规范、标准自查验次日起   " + " *" + cdvo.getf1104() + " 日内负责修复，并承 担修复费用，修复后再行交付。", setChinese(FS14), SPB28, 25.0f, 0.0f, Element.ALIGN_LEFT));
			...
			document.add(getParagraph(" 除不可抗力外，出卖人未按照第十一条约定的时间将该商品房交付买受人的，双 方同意按照下列第   " + " *" + cdvo.getf1201() + " 种方式处理 ", setChinese(FS14), SPB28, 25.0f,
					0.0f, Element.ALIGN_LEFT));
			document.add(getParagraph(" 1. 按照逾期时间，分别处理 （（1）和（2）不作累加 ） 。  ", setChinese(FS14), SPB28, 25.0f, 0.0f, Element.ALIGN_LEFT));
			document.add(getParagraph(" (1)逾期在      " + " *" + cdvo.getf1202() + " 日之内（该期限应当不多于第八条第1（1）项中的期限），自第 十一条约定的交付期限届满之次日起至实际交付之日止，出卖人按日计算向买受人支 付全部房价款万分之 " + " *" + cdvo.getf1203() + " 的违约金（该违约金比率应当不低于第八条第1（1）项中的 比率）。 ", setChinese(FS14), SPB28, 25.0f, 0.0f, Element.ALIGN_LEFT));
			document.add(getParagraph(
					" (2)逾期超过       " + " *" + cdvo.getf1204() + "  日（该期限应当与本条第（1）项中的期限相同）后，买受人 有权解除合同。买受人解除合同的，应当书面通知出卖人。出卖人应当自解除合同通 知送达之日起15日内退还买受人已付全部房款（含已付贷款部分），并自买受人付款之 日起，按照   " + " *" + cdvo.getf1205() + "  %（不低于中国人民银行公布的同期贷款基准利率）计算给付利 息；同时，出卖人按照全部房价款的  " + " *" + cdvo.getf1206()
							+ "  %向买受人支付违约金。 ", setChinese(FS14),
					SPB28, 25.0f, 0.0f, Element.ALIGN_LEFT));
			...
			document.add(getParagraph(" 该商品房交付时，出卖人应当向买受人出示房屋测绘报告，并向买受人提供该商 品房的面积实测数据（以下简称实测面积）。实测面积与第三条载明的预测面积发生误 差的，双方同意按照第     " + " *" + cdvo.getf1301() + " 种方式处理。", setChinese(FS14), SPB28, 25.0f, 0.0f, Element.ALIGN_LEFT));
			...
			document.add(getParagraph(" 买受人解除合同的，应当书面通知出卖人。出卖人应当自解除合同通知送达之日 起15日内退还买受人已付全部房款（含已付贷款部分），并自买受人付款之日起，按 照       " + " *" + cdvo.getf1302() + "  %（不低于中国人民银行公布的同期贷款基准利率）计算给付利息。 ", setChinese(FS14), SPB28, 25.0f, 0.0f, Element.ALIGN_LEFT));
			document
					.add(getParagraph(
" 买受人选择不解除合同的，实测套内建筑面积大于预测套内建筑面积时，套内建 筑面积误差比在3%以内（含3%）部分的房价款由买受人补足；超出 3% 部分的房价 款由出卖人承担，产权归买受人所有。实测套内建筑面积小于预测套内建筑面积时， 套内建筑面积误差比绝对值在3%以内（含3%）部分的房价款由出卖人返还买受人； 绝对值超出3%部分的房价款由出卖人双倍返还买受人。        ",
			...
			document.add(getParagraph(" 买受人解除合同的，应当书面通知出卖人。出卖人应当自解除合同通知送达之日 起15日内退还买受人已付全部房款（含已付贷款部分），并自买受人付款之日起，按 照  " + " *" + cdvo.getf1303() + " %（不低于中国人民银行公布的同期贷款基准利率）计算给付利息。 "));
			document
					.add(getParagraph(
...
" 双方签订合同后，涉及该商品房规划用途、面积、容积率、绿地率、基础设施、 公共服务及其他配套设施等规划许可内容经城乡规划主管部门批准变更的，出卖人应 当在变更确立之日起10日内将书面通知送达买受人。出卖人未在规定期限内通知买受 人的，买受人有权解除合同。     ",
			...
			document.add(getParagraph("(二)买受人应当在通知送达之日起15日内做出是否解除合同的书面答复。买受人 逾期未予以书面答复的，视同接受变更。        ", setChinese(FS14), SPB28, 25.0f, 0.0f,
					Element.ALIGN_LEFT));
			document.add(getParagraph("(三)买受人解除合同的，应当书面通知出卖人。出卖人应当自解除合同通知送达 之日起15日内退还买受人已付全部房款（含已付贷款部分），并自买受人付款之日起， 按照     " + " *" + cdvo.getf1401() + "  %（不低于中国人民银行公布的同期贷款基准利率）计算给付利息；同 时，出卖人按照全部房价款的  " + " *" + cdvo.getf1402() + "  %向买受人支付违约金。 "));
...
"(一)双方签订合同后，出卖人按照法定程序变更建筑工程施工图设计文件，涉及 下列可能影响买受人所购商品房质量或使用功能情形的，出卖人应当在变更确立之日 起10日内将书面通知送达买受人。出卖人未在规定期限内通知买受人的，买受人有权 解除合同。        ",
			...
			document.add(getParagraph("(二)买受人应当在通知送达之日起15日内做出是否解除合同的书面答复。买受人 逾期未予以书面答复的，视同接受变更。       ", setChinese(FS14), SPB28, 25.0f, 0.0f,
					Element.ALIGN_LEFT));
			document
.add(getParagraph("(三)买受人解除合同的，应当书面通知出卖人。出卖人应当自解除合同通知送达 之日起15日内退还买受人已付全部房款（含已付贷款部分），并自买受人付款之日起， 按照      " + " *" + cdvo.getf1504() + "  %（不低于中国人民银行公布的同期贷款基准利率）计算给付利息；同 时，出卖人按照全部房价款的  " + cdvo.getf1505() + "  %向买受人支付违约金。 ", setChinese(FS14), SPB28, 25.0f, 0.0f,
					Element.ALIGN_LEFT));
			document.add(getParagraph(" 买受人不解除合同的，有权要求出卖人赔偿由此造成的损失，双方约定如下：   ", setChinese(FS14), SPB28, 25.0f, 0.0f, Element.ALIGN_LEFT));
			...
			document.add(getParagraph(
" 出卖人承诺该商品房地基基础和主体结构合格,并符合国家及行业标准。 经检测不合格的,买受人有权解除合同。买受人解除合同的,应当书面通知出卖 人。出卖人应当自解除合同通知送达之日起15日内退还买受人已付全部房款（含已付 贷款部分）,并自买受人付款之日起,按照  " + " *" + cdvo.getf1601() + "  %（不低于中国人民银行公布的同期 贷款基准利率）计算给付利息。给买受人造成损失的，由出卖人支付 " + " *" + cdvo.getf1602()
			...
			document.add(getParagraph(" (2)经过更换、修理，仍然严重影响正常使用的，买受人有权解除合同。买受人 解除合同的，应当书面通知出卖人。出卖人应当自解除合同通知送达之日起15日内退 还买受人已付全部房款（含已付贷款部分），并自买受人付款之日起，按照        " + " *" + cdvo.getf1605() + "  % （不低于中国人民银行公布的同期贷款基准利率）计算给付利息。给买受人造成损失 的，由出卖人承担相应赔偿责任。因此而发生的检测费用由出卖人承担。 ", setChinese(FS14), SPB28, 25.0f,
					0.0f, Element.ALIGN_LEFT));
			...
			document.add(getParagraph(" 1. 该商品房室内空气质量符合       " + " *" + cdvo.getf1611() + " 标准，标准名称： " + " *" + cdvo.getf1612() + " ，标准文号： " + " *" + cdvo.getf1613() + " 。", setChinese(FS14), SPB28, 25.0f, 0.0f, Element.ALIGN_LEFT));
			document.add(getParagraph(" 该商品房为住宅的，建筑隔声情况符合      " + " *" + cdvo.getf1614() + " 标准，标准名称： " + " *" + cdvo.getf1615() + " ，标准文号： " + " *" + cdvo.getf1616() + " 。", setChinese(FS14), SPB28, 25.0f, 0.0f, Element.ALIGN_LEFT));
			document
					.add(getParagraph(
...
"(一)商品房实行保修制度。该商品房为住宅的,出卖人自该商品房交付之日起, 按照《住宅质量保证书》承诺的内容承担相应的保修责任。该商品房为非住宅的,双 方应当签订补充协议详细约定保修范围、保修期限和保修责任等内容。具体内容见附 件七。       ",
			...
			document.add(getParagraph("(三)在保修期内，买受人要求维修的书面通知送达出卖人    " + " *" + cdvo.getf1702() + "  日内，出卖人 既不履行保修义务也不提出书面异议的，买受人可以自行或委托他人进行维修，维修 费用及维修期间造成的其他损失由出卖人承担。 ", setChinese(FS14), SPB28, 25.0f, 0.0f, Element.ALIGN_LEFT));
			...
			document.add(getParagraph("(二)因出卖人的原因，买受人未能在该商品房交付之日起       " + " *" + cdvo.getf2001() + " 日内取得该商 品房的房屋所有权证书的，双方同意按照下列第 " + " *" + cdvo.getf2002() + "  种方式处理： ", setChinese(FS14), SPB28, 25.0f, 0.0f, Element.ALIGN_LEFT));
			document.add(getParagraph(" 1. 买受人有权解除合同。买受人解除合同的，应当书面通知出卖人。出卖人应当 自解除合同通知送达之日起15日内退还买受人已付全部房款(含已付贷款部分),并自 买受人付款之日起，按照   " + " *" + cdvo.getf2003() + "  %(不低于中国人民银行公布的同期贷款基准利率) 计算给付利息。买受人不解除合同的，自买受人应当完成房屋所有权登记的期限届满 之次日起至实际完成房屋所有权登记之日止,出卖人按日计算向买受人支付全部房价 款万分之  " + " *" + cdvo.getf2004()
...
.add(getParagraph("(一)出卖人依法选聘的前期物业服务企业为         " + " *" + cdvo.getf2101() + " 。", setChinese(FS14), SPB28, 25.0f, 0.0f, Element.ALIGN_LEFT));
			document.add(getParagraph(
					"(二)物业服务时间从       " + substringDate(cdvo.getf2102(), 1) + " 年 " + substringDate(cdvo.getf2102(), 2) + " 月 " + substringDate(cdvo.getf2102(), 3) + " 日到 " + substringDate(cdvo.getf2103(), 1) + " 年 " + substringDate(cdvo.getf2103(), 2) + " 月 " + substringDate(cdvo.getf2103(), 3)
							+ " 日。", setChinese(FS14), SPB28, 25.0f, 0.0f, Element.ALIGN_LEFT));
			...
			document.add(getParagraph(" 出卖人和买受人保证在本合同中记载的通讯地址、联系电话均真实有效。任何根 据本合同发出的文件，均应采用书面形式，以         " + " *" + cdvo.getf2501() + " 方式送达对方。任何一方变更通讯地址、联系电话的，应在变更之日起 " + " *" + cdvo.getf2502() + "  日 内书面通知对方。变更的一方未履行通知义务导致送达不能的，应承担相应的法律责 任。 ", setChinese(FS14), SPB28,
					25.0f, 0.0f,
					Element.ALIGN_LEFT));
...
" 出卖人对买受人信息负有保密义务。非因法律、法规规定或国家安全机关、公安 机关、检察机关、审判机关、纪检监察部门执行公务的需要，未经买受人书面同意， 出卖人及其销售人员和相关工作人员不得对外披露买受人信息，或将买受人信息用于 履行本合同之外的其他用途。         ",
			...
			document
.add(getParagraph(" 本合同自双方签字或盖章之日起生效。本合同的解除应当采用书面形式。 本合同及附件共" + " *" + cdvo.getf2901() + " 页， 一式" + " *" + cdvo.getf2902() + " 份， 其中出卖人" + " *" + cdvo.getf2903() + " 份， 买受人" + " *" + cdvo.getf2904() + " 份， [" + " *" + cdvo.getf2905() + " ]" + " *" + cdvo.getf2906() + " 份， ["
					+ " *" + cdvo.getf2907() + " ]" + " *" + cdvo.getf2908() + " 份。合同附件与本合同具有同等法律效力。", setChinese(FS14), SPB28, 25.0f, 0.0f,
							Element.ALIGN_LEFT));
...
//			hvo.setPre_Plan_ID("25000000868168");
			...
			if(palist!=null&&!palist.isEmpty()){
				...
				for(int count=0;count<palist.size();count++){
					...
					if(pa.getState() == 4){
						sb.append(count + 1).append("、").append("贷款申请时间：     *").append(pa.getFkDate()).append("    ,贷款金额：    *").append(pa.getMoneyStr()).append("   ,款项：    *").append(pa.getStateStr()).append(",贷款机构：   *").append(pa.getBankName()).append(",放款时间：  *").append(
										pa.getDkDate());
					}else{
						sb.append(count + 1).append("、").append("付款时间:     *").append(pa.getFkDate()).append("    ,付款金额：    *").append(pa.getMoneyStr()).append("   ,款项：    *").append(pa.getStateStr());
					}
...
.add(getParagraph("保修期限为：" + " *" + cdvo.getf9101() + " (不得低于设计文件规定的该工程的合理使用年限)", setChinese(FS14), SPB28, 25.0f, 0.0f, Element.ALIGN_LEFT));
	...
	public boolean pdf2Pic(String pdfPath,String path) {
		try{
			...
			/*
			 * org.icepdf.core.pobjects.Document document = new org.icepdf.core.pobjects.Document(); document.setFile(pdfPath); float scale = 2.5f;//缩放比例 float rotation = -90f;//旋转角度
			 * //取pdf第一页的内容，所以getPageImage方法的第一个参数为0 BufferedImage image = (BufferedImage) document.getPageImage(0, GraphicsRenderingHints.SCREEN, org.icepdf.core.pobjects.Page.BOUNDARY_CROPBOX,
			 * rotation, scale); RenderedImage rendImage = image; try{ String imgName = ".png"; //System.out.println(imgName); File file = new File(path + imgName); ImageIO.write(rendImage, "png",
			 * file); }catch (IOException e){ e.printStackTrace(); } image.flush(); document.dispose();
			 */
...
"3、房屋交付使用后，房屋的主体结构质量不合格的，按国务院发布的《城市房地产开发经营管理条例》第三十二条关于\"商品房交付使用后，购买人认为主体结构质量不合格的，可以向工程质量监督单位申请重新核验。经核验，确属主体结构质量不合格的，购买人有权退房；给购买人造成损失的，房地产开发企业应当依法承担赔偿责任。\"的规定处理。",
			...
			document.add(getParagraph("^第一条    甲方通过土地使用权       " + " *" + pcVo.getAttribute("landattorntype_dict_name") + "    方式取得          " + " *" + pcVo.getN1_1() + "       区/市        " + " *" + pcVo.getN1_2() + "     地块土地使用权，并依法进行了土地使用权登记取得房地产权证，证书号为：     " + " *" + pcVo.getN1_3()
					+ "    土地面积为：      " + " *" + pcVo.getN1_4() + "   平方米，土地用途为：    " + " *" + pcVo.getN1_5() + "  。"));
			document.add(getParagraph("甲方经批准，在该地块上投资建造《    " + " *" + pcVo.getN1_6() + "   》（暂定名/现定名）商品房，主体建筑物的建筑结构为    " + " *" + pcVo.getN1_7() + "   结构；建筑物地上层数为    " + " *" + pcVo.getN1_8() + "   层，地下层数为    " + " *" + pcVo.getN1_9() + "  层。"));
			document.add(getParagraph("上述商品房已具备规定的预售条件，    " + " *" + pcVo.getN1_10() + "  已批准上市预售（预售许可证编号：    " + " *" + pcVo.getN1_11() + "  ）。"));
			document.add(getParagraph("^第二条   乙方向甲方购买    " + " *" + pcVo.getN1_1() + "  区/市    " + " *" + pcVo.getN2_1() + "  路     " + " *" + pcVo.getLaneName() + "  号《     " + " *" + pcVo.getN2_3() + "  》     " + " *" + pcVo.getSubLane() + "   栋     " + " *" + pcVo.getN2_4() + "  单元     " + " *"
					+ pcVo.getN2_5() + "  层     " + " *" + pcVo.getN2_6() + "  户（以下简称该房屋），政府批准的规划用途为     " + " *" + pcVo.getN2_7() + "   。"));
			document
.add(getParagraph("据甲方暂测该房屋建筑面积为    " + " *" + pcVo.getN2_8() + "  平方米，其中套内建筑面积为    " + " *" + pcVo.getN2_9() + "  平方米、公用分摊建筑面积为    " + " *" + pcVo.getN2_10() + "  平方米，另有地下附属面积    " + " *" + pcVo.getCellarArea() + "  平方米（价格详见补充条款）。该房屋建筑层高为     " + " *" + pcVo.getN2_11() + "  米。"));
			document
.add(getParagraph("该房屋建筑设计及平面图见本合同附件二；该房屋建筑结构、装修及设备标准见本合同附件三；该房屋相关情况说明（抵押关系、资料关系）见本合同附件四，（相邻关系及小区平面布局）见本合同附件六；该房屋《前期物业管理服务合同》、《使用公约》或有关承诺书见本合同附件五。"));
			...
			if("0".equals(isNewVer)){
				...
				document.add(getParagraph("2、甲方同意当暂测面积与实测面积的误差超过+  " + " *" + pcVo.getN5_1() + " %（包括  " + " *" + pcVo.getN5_2() + " %），不向乙方收取超过部分的房价款；甲方同意当暂测面积与实测面积的误差超过－  " + " *" + pcVo.getN5_3() + " % （包括－  " + " *" + pcVo.getN5_4()
						+ " %），乙方有权单方面解除本合同。乙方行使单方解除权时，必须在双方签署《房屋交接书》之时或之前提出，否则视为放弃该项权利。", setChinese(FS14), FL28, LD21, 0.0f, Element.ALIGN_LEFT));
			}else{
				document.add(getParagraph("^第五条  在该房屋交付时，房屋建筑面积以   " + " *" + pcVo.getChargeDep() + "   认定的测绘机构实测面积为准，如甲方暂测面积与实测面积不一致时，除法律、法规、规章另有规定外按下列第 " + " *" + dicBo.getDictName("ct_509", String.valueOf(pcVo.getN5_6())) + "  项约定处理："));
				document.add(getParagraph("壹、按该房屋每平方米建筑面积单价计算多退少补。", setChinese(FS14), FL28, LD21, 0.0f, Element.ALIGN_LEFT));
				document
						.add(getParagraph(
						"贰、面积误差比在 " + " *" + getDoubleStr(pcVo.getN5_5()) + "  %以内的，据实结算房价款，甲方同意当暂测面积与实测面积的误差超过+" + " *" + getDoubleStr(pcVo.getN5_1()) + " %（包括  " + " *" + getDoubleStr(pcVo.getN5_2()) + "  %），不向乙方收取超过部分的房价款；甲方同意当暂测面积与实测面积的误差超过－  " + " *" + getDoubleStr(pcVo.getN5_3()) + " % （包括－ "
								+ " *" + getDoubleStr(pcVo.getN5_4()) + " %），乙方有权单方面解除本合同。乙方行使单方解除权时，必须在双方签署《房屋交接书》之时或之前提出，否则视为放弃该项权利。",
								setChinese(FS14), FL28, LD21, 0.0f, Element.ALIGN_LEFT));

			}
			document.add(getParagraph("^第六条  签订本合同时，该房屋建设工程建设到  " + " *" + pcVo.getN6_1() + " 。乙方应当按本合同约定时间如期足额将房价款解入甲方的预售款监管帐户（预售款监管机构：  " + " *" + pcVo.getN6_2() + " 、帐户名称  " + " *" + pcVo.getN6_3() + " 、帐号：  " + " *" + pcVo.getN6_4() + " ）。预售款按政府规定监管使用。"));
			document.add(getParagraph("乙方的付款方式和付款期限由甲乙双方在附件一中约定明确。"));
			document.add(getParagraph("^第七条   乙方若未按本合同约定的时间付款，应当向甲方支付违约金，违约金按逾期未付款额的日万分之  " + " *" + pcVo.getN7_1() + " 计算，违约金自本合同的应付款期限之第二天起算至实际付款之日止，逾期超过  " + " *" + pcVo.getN7_2() + " 天后， 甲方有权选择下列第  " + " *" + getRes(pcVo.getN7_3(), 1) + " 种方案追究乙方责任："));
			document.add(getParagraph("壹、甲方有权单方面解除本合同，乙方应当承担赔偿责任。赔偿金额为总房价款的  " + " *" + getDoubleStr(pcVo.getN7_4()) + " %，甲方有权在乙方已支付的房价款中扣除乙方应支付的赔偿金额，剩余房款退还给乙方。如乙方已支付的房价款不足赔偿的，甲方有权追索。"));
			document.add(getParagraph("甲方如行使解除合同权的，应当书面通知乙方。"));
			document.add(getParagraph("贰、  " + " *" + presellN7_5));
			document.add(getParagraph("^第八条   签订本合同后，甲方不得擅自变更该房屋的建筑设计（见附件二），确需变更的应当征得乙方书面同意并报规划管理部门审核批准，在获得批准之日起  " + " *" + pcVo.getN8_1() + " 天内与乙方签订本合同变更协议。"));
			...
			document.add(getParagraph("^第十一条   甲方定于    " + " *" + pcVo.getN11_1() + "  年    " + " *" + pcVo.getN11_2() + "  月    " + " *" + pcVo.getN11_3() + "  日前将该房屋交付给乙方，除不可抗力外。"));
			document.add(getParagraph("^第十二条    甲方如未在本合同第十一条约定期限内将该房屋交付乙方，应当向乙方支付违约金，违约金按乙方已支付的房价款日万分之  " + " *" + pcVo.getN12_1() + " 计算，违约金自本合同第十一条约定的最后交付期限之第二天起算至实际交付之日止。逾期超过  " + " *" + pcVo.getN12_2() + " 天，乙方有权选择下列第  " + " *" + getRes(pcVo.getN12_3(), 1) + " 种方案追究甲方责任："));
			document.add(getParagraph("壹、乙方有权单方面解除本合同。"));
			document.add(getParagraph("贰、"));
			document.add(getParagraph(" *" + presellN12_4));
			document.add(getParagraph("^第十三条    该房屋符合本合同第十条约定的交付条件后，甲方应在交付之日前  " + " *" + pcVo.getN13_1() + " 天书面通知乙方办理交付该房屋的手续，乙方应在收到该通知之日起  " + " *" + pcVo.getN13_2() + " 天内，会同甲方对该房屋进行验收交接。房屋交付的标志为  " + " *" + pcVo.getN13_3() + " 。"));
			document.add(getParagraph("在验收交接时，甲方应出示符合本合同第十条约定的房屋交付条件的证明文件，因该房屋用途为  " + " *" + pcVo.getN13_4() + " 用房，甲方应向乙方提供《  " + " *" + pcVo.getN13_5() + " 质量保证书》和《  " + " *" + pcVo.getN13_6() + " 使用说明书》。同时，甲方应当根据乙方要求提供实测面积的有关资料。"));
			...
			document
.add(getParagraph("甲、乙双方在签署《房屋交接书》之日起  " + " *" + pcVo.getN14_2() + " 天内，由双方依法向  " + " *" + dicBo.getDictName("ct_510", String.valueOf(pcVo.getN14_3())) + " 办理价格申报、过户申请手续、申领该房屋的房地产权证。"));
			document
.add(getParagraph("^第十五条    该房屋的风险责任自该房屋交付之日起由甲方转移给乙方。如乙方未按约定的日期办理该房屋的验收交接手续，甲方应当发出书面催告书一次。乙方未按催告书规定的日期办理该房屋的验收交接手续的，则自催告书约定的验收交接日之第二日起该房屋的风险责任转移由乙方承担。"));
			...
			document.add(getParagraph("^第十七条   甲方交付的该房屋系验收合格的房屋。如该房屋的装修、设备标准达不到本合同附件三约定的标准，乙方有权要求甲方按实际的装修、设备与约定的装修、设备差价  " + " *" + pcVo.getN17_1() + " 倍给予补偿。如主体结构不符合本合同附件三约定的标准，乙方有权单方面解除本合同。"));
			...
			document.add(getParagraph("^第十九条     乙方行使本合同条款中约定的单方面解除本合同权利时，应书面通知甲方，甲方应当在收到乙方的书面通知起  " + " *" + pcVo.getN19_1() + " 天内将乙方已支付的房价款（包括利息，利息按中国人民银行公布的同期存款利率计算）全部退还乙方，并承担赔偿责任，赔偿金额为总房价款的  " + " *" + pcVo.getN19_2() + " %，在退还房价款时一并支付给乙方。"));
			...
			if("0".equals(isNewVer)){
				document.add(getParagraph("^第二十一条   甲方或乙方对相对方行使单方面解除本合同有异议的，应在接到对方有关单方面解除本合同的书面通知之日起  " + " *" + pcVo.getN21_1() + " 天内，向按第三十二条选定的解决争议机关确认解除合同的效力。"));
			}else{
				document.add(getParagraph("^第二十一条   甲方或乙方对相对方行使单方面解除本合同有异议的，应在接到对方有关单方面解除本合同的书面通知之日起  " + " *" + pcVo.getN21_1() + " 天内，向按第三十三条选定的解决争议机关确认解除合同的效力。"));
			}
			...
			document.add(getParagraph("^第二十四条     甲方已选聘  " + " *" + pcVo.getN24_1() + " 物业公司对该房屋进行前期物业管理，并与其签订了《前期物业管理服务合同》（见附件五）。因该房屋规划用途为  " + " *" + pcVo.getN24_2() + " 用房，甲乙双方已签订了《  " + " *" + pcVo.getN24_3() + " 使用公约》（见附件五）。"));
			document.add(getParagraph("^第二十五条      乙方购买的房屋及其相应占有的土地使用权不可分离。自该房屋的房地产权利转移之日起，甲方与  " + " *" + pcVo.getN25_1() + " 签订的土地使用权  " + " *" + dicBo.getDictName("ct_507", String.valueOf(pcVo.getLandpactType())) + " 合同中约定的权利、义务和责任转移给乙方。"));
			document
.add(getParagraph("^第二十六条      本合同一方按照本合同约定向另一方送达的任何文件、回复及其他任何联系，必须用书面形式，且采用挂号邮寄或直接送达的方式， 送达本合同所列另一方的地址或另一方以本条所述方式通知更改后的地址。如以挂号邮寄的方式，在投邮后（以寄出的邮戳为准）第  " + " *" + pcVo.getN27_1() + " 日将被视为已送达另一方，如以直接送达的方式送达，则于另一方签收时视为已送达。"));
			...
			if("0".equals(isNewVer)){
				document.add(getParagraph("^第三十条        本合同自双方签字/  " + " *" + pcVo.getN31_1() + " 公证处公证之日起生效。双方商定本合同生效之日起  " + " *" + pcVo.getN31_2() + " 日内由  " + " *" + pcVo.getAttribute("regtype_dict_name") + " 负责向房地产登记机构办理本合同登记备案手续。"));
			...
			document.add(getParagraph("^第三十一条     本合同登记备案后，如发生协议解除本合同的事实时，在事实发生之日起30天内双方持解除合同的书面文件到  " + " *" + dicBo.getDictName("ct_510", String.valueOf(pcVo.getN32_1())) + " 办理注销本合同登记备案的手续。"));
			if("0".equals(isNewVer)){
				...
				document.add(getParagraph("^第三十三条      本合同壹式  " + " *" + pcVo.getN34_1() + " 份，均具有同等效力。其中甲、乙双方各执  " + " *" + pcVo.getN34_2() + " 份，  " + " *" + pcVo.getN34_3() + " 、  " + " *" + pcVo.getN34_4() + " 、  " + " *" + pcVo.getN34_5() + " 各执壹份"));
			}else{
				document
.add(getParagraph("^第三十二条    甲、乙双方选择  " + " *" + dicBo.getDictName("ct_509", String.valueOf(pcVo.getN32_2())) + " 自本合同生效之日起  " + " *" + pcVo.getN31_2() + " 日内向房地产登记机构申请预购商品房预告登记。"));
				...
				document.add(getParagraph("壹、向 " + " *" + pcVo.getN33_2() + " 仲裁委员会申请仲裁；", setChinese(FS14), FL28, LD21, 0.0f, Element.ALIGN_LEFT));
				document.add(getParagraph("贰、依法向人民法院起诉。", setChinese(FS14), FL28, LD21, 0.0f, Element.ALIGN_LEFT));
				document.add(getParagraph("^第三十四条    本合同壹式  " + " *" + pcVo.getN34_1() + " 份，均具有同等效力。其中甲、乙双方各执  " + " *" + pcVo.getN34_2() + " 份，  " + " *" + pcVo.getN34_3() + " 、  " + " *" + pcVo.getN34_4() + " 、  " + " *" + pcVo.getN34_5() + " 各执壹份"));
			}
...
new String[] { "住址：", "*" + baseHandle(bvo.getBuyerAddress()), " 邮编：", " *" + baseHandle(bvo.getBuyerPostcode()) }, 9, 67, 9,
				...
				//设表的宽度为523f，一行37个字，一个字占2.7%
				document.add(getParagraph("^第一条 项目建设依据"));
				document.add(getParagraph("1. 出卖人以 " + " *" + cs.getf0101() + " 方式取得坐落于" + " *" + cs.getf0102() + " 地块的建设用地使用权。该地块 " + " *" + cs.getf0103() + " 为" + " *" + cs.getf0104() + " ," + "土地使用权面积为" + " *" + cs.getf0105() + " 平方米。买受人购买的商品房（以下简称该商" + "品 房）所占用的土地用途为" + " *" + cs.getf0106()
						+ " ,土地使用权终止日期为" + " *" + cs.getf0107() + " 。", setChinese(FS14), SPB28, 25.0f, 0.0f, Element.ALIGN_LEFT));
				document.add(getParagraph("2. 出卖人经批准，在上述地块上建设的商品房项目核准名称为" + " *" + cs.getf0108() + " ， 建设工程规划许可证号为" + " *" + cs.getf0109() + " ， 建筑工程施工许可证号为" + " *" + cs.getf0110() + " 。", setChinese(FS14), SPB28, 25.0f, 0.0f, Element.ALIGN_LEFT));
				...
				document.add(getParagraph("2.该商品房所在建筑物的主体结构为" + " *" + cs.getf0302() + " ， 建筑总层数为" + " *" + cs.getf0303() + " 层， 其中地上" + " *" + cs.getf0304() + " 层， 地下" + " *" + cs.getf0305() + " 层。", setChinese(FS14), SPB28, 25.0f, 0.0f, Element.ALIGN_LEFT));
				document.add(getParagraph(" 3.该商品房为第一条规定项目中的" + " *" + cs.getf0306() + " 。该商品房的平面图见附件一。", setChinese(FS14), SPB28, 25.0f, 0.0f,
						Element.ALIGN_LEFT));
				document.add(getParagraph(" 4.该商品房的房产测绘机构为" + " *" + cs.getf0307() + " ， 其实测建筑面积共" + " *" + cs.getf0308() + " 平方米， 其中套内建筑面积" + " *" + cs.getf0309() + " 平方米， 分摊共有建筑面积" + " *" + cs.getf0310() + " 平方米。该商品房共用部位见附件二。 " + " 该商品房层高为" + " *" + cs.getf0311() + " 米， 有" + " *" + cs.getf0312()
						+ " 个阳台， 其中" + " *" + cs.getf0313() + " 个阳台为封闭式，" + " *" + cs.getf0314() + " 个阳台为非封闭式。阳台是否封闭以规划设计文件为准。 ",
						setChinese(FS14), SPB28, 25.0f, 0.0f, Element.ALIGN_LEFT));

				document.add(getParagraph("^第四条 抵押情况 "));
				document.add(getParagraph("与该商品房有关的抵押情况为" + " *" + cs.getf0401() + "  ， 抵押人：" + " *" + cs.getf0402() + " ， 抵押权人" + " *" + cs.getf0403() + " ， 抵押登记机构：" + " *" + cs.getf0404() + " ， 抵押登记日期：" + " *" + cs.getf0405() + " ， 债务履行期限：" + " *" + cs.getf0406()
				...
				document.add(getParagraph("租赁期限：从" + substringDate(cs.getf0503(), 1) + " 年" + substringDate(cs.getf0503(), 2) + " 月" + substringDate(cs.getf0503(), 3) + " 日" + " 至" + substringDate(cs.getf0504(), 1) + " 年" + substringDate(cs.getf0504(), 2) + " 月" + substringDate(cs.getf0504(), 3)
						+ " 日" + " 。出卖人与买受人经协商一致，自本合同约定的交付日至租赁期限届满期间的房屋收益归" + " *" + cs.getf0505() + " 所有。",
						setChinese(FS14), SPB28, 25.0f, 0.0f, Element.ALIGN_LEFT));
				...
				document.add(getParagraph(
" 如该商品房权利状况与上述情况不符，导致不能完成房屋所有权转移登记的，买受人有权解除合同。买受人解除合同的，应当书面通知出卖人。出卖人应当自解除合同通知送达之日起15日内退还买受人已付全部房款（含已付贷款部分），并自买受人付款之日起， 按照" + " *" + cs.getf0603() + " %(不低于中国人民银行公布的同期贷款基准利率）计算给付利息。给买受人造成损失的，由出卖人支付" + " *" + cs.getf0604() + " 的赔偿金。 ", setChinese(FS14),
						SPB28, 25.0f, 0.0f, Element.ALIGN_LEFT));
				document.newPage();
				...
				document.add(getParagraph("1.按照套内建筑面积计算，该商品房单价（不包含房屋装修）为每平方米         *" + cs.getf0715() + "  （币种）         *" + cs.getf0702() + "  元， 总价款为（不包含房屋装修）    *" + cs.getf0716() + " （币种）        *" + cs.getf0703() + " 元 （大写     *" + cs.getf0704() + " 元整）。 该商品房装修价格为每平方米      *" + cs.getf0717()
						+ " （币种）" + " *" + cs.getf0718() + " 元， 装修总价为人民币          *" + cs.getf0719() + " 元（大写 " + " *" + cs.getf0726() + " 元整）。",
						setChinese(FS14), SPB28, 25.0f, 0.0f, Element.ALIGN_LEFT));
				document.add(getParagraph("2.按照建筑面积计算，该商品房单价（不包含房屋装修）为每平方米       *" + cs.getf0720() + "  （币种）" + " *" + cs.getf0705() + " 元， 总价款（不包含房屋装修）为        *" + cs.getf0721() + "  （币种）" + " *" + cs.getf0706() + " 元 （大写" + " *" + cs.getf0707() + " 元整）。 该商品房装修价格为每平方米        *" + cs.getf0722()
						+ "  （币种）" + " *" + cs.getf0708() + " 元， 装修总价为人民币" + " *" + cs.getf0709() + " 元（大写 " + " *" + cs.getf0727() + " 元整）。",
						setChinese(FS14), SPB28, 25.0f, 0.0f, Element.ALIGN_LEFT));
				document.add(getParagraph("3.按照套计算，该商品房总价款为       *" + cs.getf0723() + "  （币种）" + " *" + cs.getf0710() + " 元 （大写" + " *" + cs.getf0711() + " 元整）。 装修总价为人民币" + " *" + cs.getf0724() + " 元（大写 " + " *" + cs.getf0738() + " 元整）。", setChinese(FS14),
						SPB28, 25.0f, 0.0f, Element.ALIGN_LEFT));
				document.add(getParagraph("4.按照" + " *" + cs.getf0712() + " 计算， 该商品房总价款为        *" + cs.getf0725() + "  （币种）" + " *" + cs.getf0713() + " 元 （大写" + " *" + cs.getf0714() + " 元整）。", setChinese(FS14), SPB28, 25.0f, 0.0f, Element.ALIGN_LEFT));
				document.add(getParagraph("5.按照套内建筑面积计算，该商品房单价为每平方米         *" + cs.getf0728() + "  （币种）         *" + cs.getf0729() + "  元， 总价款为    *" + cs.getf0730() + " （币种）        *" + cs.getf0731() + " 元 （大写     *" + cs.getf0732() + " 元整）。", setChinese(FS14), SPB28, 25.0f, 0.0f,
						Element.ALIGN_LEFT));
				document.add(getParagraph("6.按照建筑面积计算，该商品房单价为每平方米         *" + cs.getf0733() + "  （币种）         *" + cs.getf0734() + "  元， 总价款为    *" + cs.getf0735() + " （币种）        *" + cs.getf0736() + " 元 （大写     *" + cs.getf0737() + " 元整）。", setChinese(FS14), SPB28, 25.0f, 0.0f,
						Element.ALIGN_LEFT));
				
				document.add(getParagraph("^第八条 付款方式及期限 "));
				document.add(getParagraph("（一）签订本合同前，买受人已向出卖人支付定金 人民币  （币种）" + " *" + cs.getf0801() + " 元 （大写" + " *" + cs.getf0802() + " 元整）， 该定金于" + " *" + cs.getf0803() + " 时" + " *" + cs.getf0805() + " 商品房价款。 ", setChinese(FS14), SPB28, 25.0f, 0.0f, Element.ALIGN_LEFT));
				document.add(getParagraph("(二）买受人采取下列第" + " *" + cs.getf0807() + " 种方式付款： ", setChinese(FS14), SPB28, 25.0f, 0.0f, Element.ALIGN_LEFT));
				document.add(getParagraph("1.一次性付款。买受人应当在" + substringDate(cs.getf0808(), 1) + " 年" + substringDate(cs.getf0808(), 2) + " 月" + substringDate(cs.getf0808(), 3) + " 日前支付该商品房全部价款。 ", setChinese(FS14), SPB28, 25.0f, 0.0f, Element.ALIGN_LEFT));
				document.add(getParagraph("2.分期付款。买受人应当在" + substringDate(cs.getf0809(), 1) + " 年" + substringDate(cs.getf0809(), 2) + " 月" + substringDate(cs.getf0809(), 3) + " 日前分" + " *" + cs.getf0810() + " 期支付该商品房全部价款， 首期房价款人民币  （币种）" + " *" + cs.getf0811() + " 元 （大写" + " *" + cs.getf0812()
						+ " 元整）， 应当于" + " *" + cs.getf0813() + " 日前支付。 ", setChinese(FS14), SPB28, 25.0f, 0.0f,
						Element.ALIGN_LEFT));
				document.add(getParagraph(" *" + cs.getf0814(), setChinese(FS14), SPB28, 25.0f, 0.0f, Element.ALIGN_LEFT));
				document.add(getParagraph("3.贷款方式付款：" + " *" + cs.getf0815() + " 。 买受人应当于" + substringDate(cs.getf0817(), 1) + " 年" + substringDate(cs.getf0817(), 2) + " 月" + substringDate(cs.getf0817(), 3) + " 日前 支付首期房价款人民币  （币种）" + " *" + cs.getf0818() + " 元 （大写" + " *" + cs.getf0819()
						+ " 元整）， 占全部房价款的" + " *" + cs.getf0820() + " ％。", setChinese(FS14), SPB28, 25.0f, 0.0f,
						Element.ALIGN_LEFT));
				document.add(getParagraph(" 余款人民币  （币种）" + " *" + cs.getf0821() + " 元 （大写" + " *" + cs.getf0822() + " 元整）， 向" + " *" + cs.getf0823() + " （贷款机构）申请贷款支付。 ", setChinese(FS14), SPB28, 25.0f, 0.0f, Element.ALIGN_LEFT));
				...
				document.add(getParagraph("（1）逾期在" + " *" + cs.getf0902() + " 日之内， 买受人按日计算向出卖人支付逾期应付款万分之" + " *" + cs.getf0903() + " 的违约金。 ", setChinese(FS14),
						SPB28, 25.0f, 0.0f, Element.ALIGN_LEFT));
				document.add(getParagraph("（2）逾期超过" + " *" + cs.getf0904() + " 日（该期限应当与本条第（1）项中的期限相同）后，出卖人有 权解除合同。出卖人解除合同的，应当书面通知买受人。 买受人应当自解除合同通知送达之日起" + " *" + cs.getf0905() + " 日内按照累计应付款的" + " *" + cs.getf0906() + " %向出卖人支付违约金，同时，出卖人退还买受人已付全部房款（含已付贷款部分）。", setChinese(FS14), SPB28, 25.0f,
						0.0f,
						Element.ALIGN_LEFT));
...
.add(getParagraph("（1）以上设施中第1、2、3、4项在约定交付日未达到交付条件的，出卖人按照本合同第十三条的约定承担逾期交付责任。 ", setChinese(FS14), SPB28, 25.0f, 0.0f, Element.ALIGN_LEFT));
				document.add(getParagraph("第5项未按时达到交付使用条件的，出卖人按日向买受人支付" + " *" + cs.getf1106() + " 元的违约金；第6项未按时达到交付使用条件的， 出卖人按日向买受人支付" + " *" + cs.getf1107() + " 元的违约金； 第7项未按时达到交付使用条件的，出卖人按日向买受人支付" + " *" + cs.getf1108() + " 元的违约金。 出卖人采取措施保证相关设施于约定交付日后" + " *" + cs.getf1109() + " 日之内达到交付使用条件。 ",
						setChinese(FS14), SPB28, 25.0f, 0.0f, Element.ALIGN_LEFT));
				document.add(getParagraph("（2）" + " *" + cs.getf1110(), setChinese(FS14), SPB28, 25.0f, 0.0f, Element.ALIGN_LEFT));
				document.add(getParagraph("（二）公共服务及其他配套设施（以建设工程规划许可为准） ", setChinese(FS14), SPB28, 25.0f, 0.0f, Element.ALIGN_LEFT));
				document.add(getParagraph("1.小区内绿地率：" + substringDate(cs.getf1111(), 1) + " 年" + substringDate(cs.getf1111(), 2) + " 月" + substringDate(cs.getf1111(), 3) + " 日达到" + " *" + cs.getf1112() + " ；", setChinese(FS14), SPB28, 25.0f, 0.0f, Element.ALIGN_LEFT));
				document.add(getParagraph("2.小区内非市政道路：" + substringDate(cs.getf1113(), 1) + " 年" + substringDate(cs.getf1113(), 2) + " 月" + substringDate(cs.getf1113(), 3) + " 日达到：" + " *" + cs.getf1114() + " ；", setChinese(FS14), SPB28, 25.0f, 0.0f, Element.ALIGN_LEFT));
				document.add(getParagraph("3.规划的车位、车库：" + substringDate(cs.getf1115(), 1) + " 年" + substringDate(cs.getf1115(), 2) + " 月" + substringDate(cs.getf1115(), 3) + " 日达到：" + " *" + cs.getf1116() + " ；", setChinese(FS14), SPB28, 25.0f, 0.0f, Element.ALIGN_LEFT));
				document.add(getParagraph("4.物业服务用房：" + substringDate(cs.getf1117(), 1) + " 年" + substringDate(cs.getf1117(), 2) + " 月" + substringDate(cs.getf1117(), 3) + " 日达到：" + " *" + cs.getf1118() + " ；", setChinese(FS14), SPB28, 25.0f, 0.0f, Element.ALIGN_LEFT));
				document.add(getParagraph("5、医疗卫生机构：" + substringDate(cs.getf1119(), 1) + " 年" + substringDate(cs.getf1119(), 2) + " 月" + substringDate(cs.getf1119(), 3) + " 日达到：" + " *" + cs.getf1120() + " ；", setChinese(FS14), SPB28, 25.0f, 0.0f, Element.ALIGN_LEFT));
				document.add(getParagraph("6.幼儿园：" + substringDate(cs.getf1121(), 1) + " 年" + substringDate(cs.getf1121(), 2) + " 月" + substringDate(cs.getf1121(), 3) + " 日达到：" + " *" + cs.getf1122() + " ；", setChinese(FS14), SPB28, 25.0f, 0.0f, Element.ALIGN_LEFT));
				document.add(getParagraph("7.学校：" + substringDate(cs.getf1123(), 1) + " 年" + substringDate(cs.getf1123(), 2) + " 月" + substringDate(cs.getf1123(), 3) + " 日达到：" + " *" + cs.getf1124() + " ；", setChinese(FS14), SPB28, 25.0f, 0.0f, Element.ALIGN_LEFT));
				...
				document.add(getParagraph("(一)出卖人应当在" + substringDate(cs.getf1201(), 1) + " 年" + substringDate(cs.getf1201(), 2) + " 月" + substringDate(cs.getf1201(), 3) + " 日前向买受人交付该商品房。 ", setChinese(FS14), SPB28, 25.0f, 0.0f, Element.ALIGN_LEFT));
				document.add(getParagraph("(二)该商品房达到第十条、第十一条约定的交付条件后，出卖人应当在交付日期届满前" + " *" + cs.getf1202() + " 日（不少于10日）将查验房屋的时间、办理交付手续的时间地点以及应当携带的证件材料的通知书面送达买受人。买受人未收到交付通知书的，以本合同约定 的交付日期届满之日为办理交付手续的时间，以该商品房所在地为办理交付手续的地点。 ", setChinese(FS14),
						SPB28, 25.0f, 0.0f, Element.ALIGN_LEFT));
...
"2.买受人查验的该商品房存在下列除地基基础和主体结构外的其他质量问题的， 由出卖人按照有关工程和产品质量规范、标准自查验次日起" + " *" + cs.getf1204() + " 日内负责修复，并承担修复费用，修复后再行交付。",
				...
				document.add(getParagraph("（除不可抗力外，出卖人未按照第十二条约定的时间将该商品房交付买受人的，双 方同意按照下列第" + " *" + cs.getf1301() + " 种方式处理：", setChinese(FS14), SPB28, 25.0f,
						0.0f, Element.ALIGN_LEFT));
				document.add(getParagraph("1.按照逾期时间，分别处理（（1）和（2）不作累加）。 ", setChinese(FS14), SPB28, 25.0f, 0.0f, Element.ALIGN_LEFT));
				document.add(getParagraph("（1）逾期在" + " *" + cs.getf1302() + " 日之内（该期限应当不多于第九条第1 （1）项中的期限），自第 十二条约定的交付期限届满之次日起至实际交付之日止，出卖人按日计算向买受人支付全部房价款万分之" + " *" + cs.getf1303() + " 的违约金（该违约金比率应当不低于第九条第1 （1）项中的比率）。", setChinese(FS14), SPB28, 25.0f, 0.0f, Element.ALIGN_LEFT));
				document.add(getParagraph("（2）逾期超过" + " *" + cs.getf1304() + " 日（该期限应当与本条第（1）项中的期限相同）后，买受人有权解除合同。买受人解除合同的，应当书面通知出卖人。出卖人应当自解除合同通知送达之日起15日内退还买受人已付全部房款（含已付贷款部分）， 并自买受人付款之日起，按照" + " *" + cs.getf1305() + " %（不低于中国人民银行公布的同期贷款基准利率）计算给付利息； 同时，出卖人按照全部房价款的" + " *" + cs.getf1306()
						+ " %向买受人支付违约金。", setChinese(FS14), SPB28,
						25.0f, 0.0f, Element.ALIGN_LEFT));
				...
				document.add(getParagraph("经检测不合格的，买受人有权解除合同。买受人解除合同的，应当书面通知出卖 人。出卖人应当自解除合同通知送达之日起15日内退还买受人已付全部房款（含已付 贷款部分），并自买受人付款之日起， 按照" + " *" + cs.getf1401() + " %（不低于中国人民银行公布的同 期贷款基准利率）计算给付利息。给买受人造成损失的，由出卖人支付" + " *" + cs.getf1402() + " 的赔偿金。因此而发生的检测费用由出卖人承担。",
				...
				document.add(getParagraph("（2）经过更换、修理，仍然严重影响正常使用的，买受人有权解除合同。买受人解除合同的，应当书面通知出卖人。出卖人应当自解除合同通知送达之日起15日内退 还买受人已付全部房款（含已付贷款部分），并自买受人付款之日起， 按照" + " *" + cs.getf1405() + " %（不低于中国人民银行公布的同期贷款基准利率）计算给付利息。给买受人造成损失 的，由出卖人承担相应赔偿责任。因此而发生的检测费用由出卖人承担。", setChinese(FS14), SPB28, 25.0f,
						0.0f,
						Element.ALIGN_LEFT));
				...
				document.add(getParagraph("（四）室内空气质量、建筑隔声和民用建筑节能措施", setChinese(FS14), SPB28, 25.0f, 0.0f, Element.ALIGN_LEFT));
				document.add(getParagraph("1.该商品房室内空气质量符合" + " *" + cs.getf1410() + "   标准， 标准名称：" + " *" + cs.getf1411() + " ， 标准文号：" + " *" + cs.getf1412() + " 。", setChinese(FS14), SPB28, 25.0f, 0.0f, Element.ALIGN_LEFT));
				document.add(getParagraph("该商品房为住宅的，建筑隔声情况符合" + " *" + cs.getf1413() + "   标准， 标准名称：" + " *" + cs.getf1414() + " ， 标准文号：" + " *" + cs.getf1415() + " 。", setChinese(FS14), SPB28, 25.0f, 0.0f, Element.ALIGN_LEFT));
				document
						.add(getParagraph(
"该商品房室内空气质量或建筑隔声情况经检测不符合标准，由出卖人负责整改，整改后仍不符合标准的，买受人有权解除合同。买受人解除合同的，应当书面通知出 卖人。出卖人应当自解除合同通知送达之日起15日内退还买受人已付全部房款（含已付贷款部分），并自买受人付款之日起， 按照"
				...
				document.add(getParagraph("（三）在保修期内，买受人要求维修的书面通知送达出卖人" + " *" + cs.getf1502() + " 日内，出卖人既不履行保修义务也不提出书面异议的，买受人可以自行或委托他人进行维修，维修费用及维修期间造成的其他损失由出卖人承担。", setChinese(FS14), SPB28, 25.0f, 0.0f, Element.ALIGN_LEFT));
				...
				document.add(getParagraph("（一）双方同意共同向房屋登记机构申请办理该商品房的房屋所有权转移登记。", setChinese(FS14), SPB28, 25.0f, 0.0f, Element.ALIGN_LEFT));
				document.add(getParagraph("（二）因出卖人的原因，买受人未能在该商品房交付之日起" + " *" + cs.getf1701() + " 日内取得该商品房的房屋所有权证书的， 双方同意按照下列第" + " *" + cs.getf1702() + " 种方式处理： ", setChinese(FS14), SPB28, 25.0f, 0.0f, Element.ALIGN_LEFT));
				document.add(getParagraph(
						"1.买受人有权解除合同。买受人解除合同的，应当书面通知出卖人。出卖人应当自解除合同通知送达之日起15日内退还买受人已付全部房款（含已付贷款部分），并自 买受人付款之日起， 按照" + " *" + cs.getf1703() + " % （不低于中国人民银行公布的同期贷款基准利率） 计算给付利息。买受人不解除合同的，自买受人应当完成房屋所有权登记的期限届满之次日起至实际完成房屋所有权登记之日止， 出卖人按日计算向买受人支付全部房价款万分之" + " *" + cs.getf1704() + " 的违约金。",
						setChinese(FS14), SPB28, 25.0f, 0.0f, Element.ALIGN_LEFT));
				...
				document.add(getParagraph("(二)物业服务时间从" + substringDate(cs.getf1802(), 1) + " 年" + substringDate(cs.getf1802(), 2) + " 月" + substringDate(cs.getf1802(), 3) + " 日到" + substringDate(cs.getf1803(), 1) + " 年" + substringDate(cs.getf1803(), 2) + " 月" + substringDate(cs.getf1803(), 3)
						+ " 日。", setChinese(FS14), SPB28, 25.0f, 0.0f, Element.ALIGN_LEFT));
				...
				document.add(getParagraph("出卖人和买受人保证在本合同中记载的通讯地址、联系电话均真实有效。任何根据本合同发出的文件，均应采用书面形式，以" + " *" + cs.getf2201() + " 方式送达对方。任何一方变更通讯地址、联系电话的， 应在变更之日起" + " *" + cs.getf2202() + " 日内书面通知对方。变更的一方未履行通知义务导致送达不能的，应承担相应的法律责任。", setChinese(FS14), SPB28,
						25.0f, 0.0f, Element.ALIGN_LEFT));
...
"出卖人对买受人信息负有保密义务。非因法律、法规规定或国家安全机关、公安机关、检察机关、审判机关、纪检监察部门执行公务的需要，未经买受人书面同意， 出卖人及其销售人员和相关工作人员不得对外披露买受人信息，或将买受人信息用于履行本合同之外的其他用途。",
				...
				document.add(getParagraph(
						"本合同自双方签字或盖章之日起生效。本合同的解除应当采用书面形式。 本合同及附件共" + " *" + cs.getf2601() + " 页， 一式" + " *" + cs.getf2602() + " 份， 其中出卖人" + " *" + cs.getf2603() + " 份， 买受人" + " *" + cs.getf2604() + " 份， 【" + " *" + cs.getf2605() + " 】 " + " *" + cs.getf2606() + " 份， 【" + " *" + cs.getf2607()
								+ " 】 " + " *" + cs.getf2608() + " 份。合同附件与本合同具有同等法律效力。", setChinese(FS14), SPB28, 25.0f, 0.0f,
						Element.ALIGN_LEFT));
