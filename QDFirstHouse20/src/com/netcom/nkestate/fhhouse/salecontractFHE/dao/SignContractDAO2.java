 ...
 * 签约管理模块DAO
 */
public class SignContractDAO2 extends MiniDAO {

	static Logger logger = Logger.getLogger(SignContractDAO2.class.getName());
	...
	public int deleteYF(BuyerInfoVOFHE biVo) throws Exception {
		try{
			StringBuffer sb = new StringBuffer();
			List<Object> params = new ArrayList<Object>();
			sb.append("	delete from SS_CONTRACT_FHE_BUYER a where a.contract_id = ? and a.serial = ?  ");
			...
			//result = 1;
			if(result > 0){
				int result2 = updateYF(contractId, serial);
			}
			return result;
		}catch (Exception e){
			e.printStackTrace();
	...
	public int updateYF(long contract_id,int serial) throws Exception { //serial=2
		try{
			StringBuffer sb = new StringBuffer();
			List<Object> params = new ArrayList<Object>();
			int result = 1;
				sb.append("	update SS_CONTRACT_FHE_BUYER b set b.serial=b.serial-1 where b.contract_id = ? and b.serial > ?");
				params.add(contract_id);
			params.add(serial);
				result = DataBaseUtil.execute(sb.toString(), params, conn);
			return result;
